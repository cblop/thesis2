...
\Fref{cha:evaluation}.}{2.3.2c: Rephrased to explain why we're looking at interactive narrative}
...
\Fref{sec:pjexample} contains a more detailed description of the InstAL
...
Section~\ref{sec:instal-obl} on page~\pageref{sec:instal-obj} explains in detail how obligations, deadlines and
...
section~\Fref{sec:combining-tropes} of \Fref{cha:storybuilder}.
...
Cliffe's model specifies three types of \textbf{event}: \emph{external events} (or `observed events', $\mathcal{E}_{obs}$), \emph{institutional events} ($\mathcal{E}_{instevent}$) and \emph{violation events} ($\mathcal{E}_{viol}$). Examples of each are given in \Fref{fig:events-pj}.
...
caption={Hash Map representation of the ``MacGuffin trope from \Fref{lst:macguffin}''}]
{:label "MacGuffin",
...
\added{Further discussion on trope combination appears in \Fref{sec:trope-combination-exp}.}{2.3.3: Forward ref to discussion on
...
Issue~\ref{iss:freedom} from page~\pageref{iss:freedom} the literature review. An institution is a
...
Using a social institution as a story model addresses Issue~\ref{iss:freedom} on
page~\pageref{iss:freedom} by
...
existing narrative formalism (\Fref{sec:stories-logic}). The same scene
...
In \Fref{sec:pjexample}, we describe the \emph{sausages} scene of Punch
...
\Fref{fig:pj-phase-terms}. The left side of Rule~\Fref{eq:phase-inactive}
...
\Fref{lst:response}, the $\texttt{+!changeMood}$ plan on
...
\emph{absentation} function, shown in \Fref{lst:absentation}. InstAL rules like those shown above and in \Fref{lst:absentation} are compiled into AnsProlog, then we use the \emph{clingo} answer set solver~\citep{gebser2011potassco} to ground the program, and `solve' queries by finding all permissions and obligations that apply to any agents, given a sequence of events as the query input. The agents' percepts are then updated with their permitted and obliged actions from that time instant onwards.  Thus, the institutional model acts as a social narrative sensor, interpreting actors' actions in the context of the combination of the concrete narrative and the abstract story moves which detach (instantiate) the norms that guide the actors in the direction of the conclusion of the story arc.
...
institution, as determined by the generates relation. \Fref{lst:asp}
...
certain instants when they have some effect on the scene, although in practice, all role fluents hold throughout the scene. \Fref{fig:tracevis} shows a visualisation of the answer set for the trace in \Fref{lst:trace}.
...
\Fref{sec:litrev-discussion}), the advantages of this approach are:
...
comes with current approaches. For this reason, \Fref{cha:tropes} explains how
...
natural-language system for defining story components, which we describe in \Fref{cha:tropical}.
...
\Fref{lst:pj-emotion} shows the shared plan library for agent emotions,
...
\Fref{sec:future-adress}. In the first Section of this chapter, however,
