...
tropes.
More detailed explanations appear in Section~\fref{sec:tropes-intro} and Chapter~\fref{cha:tropes}.
...
\added{This dissertation proposes a method for authors to construct non-linear
  ...
  Chapters~\fref{cha:tropical} and~\fref{cha:storybuilder} respectively.}{2.2.1: Added
...
Chapter~\fref{cha:evaluation}.}{2.3.2c: Rephrased to explain why we're looking at interactive narrative}
...
described in Section~\fref{sec:punchjudy}.

The Punch and Judy show in Section~\fref{sec:punchjudy} also makes use of a
...
other narrative models described in the \emph{Literature Review} chapter (Chapter~\fref{cha:literature-review}).
...
In Chapter~\fref{cha:tropical}, we describe TropICAL, our programming language
...
In order to satisfy requirement~\fref{req1}, TropICAL uses a controlled
...
branches (requirement~\fref{req2}). Requirement~\fref{req3}, the mechanism for
...
\item The use of \textbf{Institutions as Story Worlds} is described in
  ...
  the \textbf{[CMN 2015]} conference paper in the list of papers below (section~\fref{sec:papers}).
\item Using \textbf{Tropes as Story Components}
  ...
  Chapter~\fref{cha:institutions} by showing how tropes can be used as a
...
%     2015]} paper.
\item We use the requirements that emerge from Chapters~\fref{cha:literature-review}
...
Figure~\fref{fig:vonnegut} shows two examples of story shapes. In the ``Man in
...
overview of these implementations and their underlying research in Section~\fref{sec:implementations}. Has narrative theory research informed the creation of computer-generated or interactive narrative at all, or do they all take approaches grounded in computer science and artificial intelligence? If narrative theory has not been used, then we must ask: why not?
...
\changed{Chapter~\fref{cha:tropical} addresses this issue by describing how
...
grammatical generation of natural language. Figure~\fref{fig:rumelhart} shows two
...
Figure~\fref{fig:mimesis} shows how these components work together to form the
...
screenshot from the game is shown in Figure~\fref{fig:facade-screen}.
...
in Section~\fref{sec:narrative-types}) that interactive narratives must be social
...
of an exclusion operator (``!''). Listing~\fref{lst:exclusion1} shows an example
...
logic in Listing~\fref{lst:exclusion1} has a type checker which ensures that no character can have multiple genders.
...
Listing~\fref{lst:exclusion2} is another piece of \emph{Praxis} code showing how the attributes of a character called
...
Listing~\fref{lst:ceptre-murder} shows an example from~\citet{martens2015ceptre}
...
Section~\fref{sec:kripke}, where we use Interval Temporal Logic and Kripke structures to
...
Chapter~\fref{cha:institutions}) because they regulate agent behaviour without
...
Section~\fref{sec:pjexample} contains a more detailed description of the InstAL
...
InstAL syntax in Section~\fref{sec:arch-mas}.
...
institution described in Section~\fref{sec:lit-insts} is so that the characters
...
Russell's original paper proposes a model similar to that shown in Figure~\fref{fig:circumplex}, where the $x$ axis is a person's valence level and the $y$ axis is their arousal level. He argues that the full range of human emotions lie as points along these axes. Eight such examples are shown in Figure~\fref{fig:circumplex}.
...
dimension, and so left it in our emotional model. This is the approach we take in creating my Punch and Judy simulation, and so it is described in more detail in Section~\fref{sec:emotion}.
...
Though the Punch and Judy simulation described in Section~\fref{sec:punchjudy}
...
described in Section~\fref{sec:formalisms}, none have been expressive enough to
...
Section~\fref{sec:formalisms}, none have any means of abstraction such as this.
...
our system, described in Chapter~\fref{cha:tropes}.
...
metaphor (described in Section~\fref{sec:structure}, sometimes the simplest
explanations are the easiest to grasp and most enduring. We believe that our
``trope'' analogy for modelling stories (Chapter~\fref{cha:tropes}) is an
...
concept (sample size 19, see Section~\fref{sec:tropes-simple}). This means that tropes are a
...
described in Chapters~\fref{cha:tropes} and~\fref{cha:tropical} of this dissertation.

% Chapter~\fref{cha:tropes} describes
% the trope concept further, with examples. Chapter~\fref{cha:institutions}
...
% Chapter~\fref{cha:tropical}. Section~\fref{sec:library} describes our creation of
% a library of tropes and section~ The thesis concludes with an evaluation
% in section~\fref{sec:evaluation} and conclusions in section~\fref{sec:conclusions}.
...
reuse existing story fragments. We describe this is more detail in Section~\fref{sec:abstraction}.

\deleted{Three paragraphs that refer to deleted goals from the literature review}

\changed{The literature review in Chapter~\fref{cha:literature-review} highlights the need
...
Chapter~\fref{cha:tropical}.}{Refers back to revised goals in the literature review}
...
chapter~\fref{cha:introduction}. It is a template which is repeated so often in
...
Figure~\fref{fig:evil-empire} shows a screenshot of the ``The Empire'' page on the
...
(Figure~\fref{fig:periodic-table}) arranges the tropes into different ``groups''
...
atoms, as shown in the various examples in Figure~\fref{fig:periodic-examples}.
...
section~\fref{sec:litrev-discussion}, we now describe how tropes are suitable for use
...
The example we described in Section~\fref{sec:litrev-discussion}
...
\textbf{Three Act Structure} (also known as Freytag's Pyramid) shown in Figure~\fref{fig:freytag}, which describes the shape of a
...
section~\fref{sec:structure}, we can use the ``subtropes'' we just identified to
define other ``story shapes'' as described by~\citet{vonnegut2009palm}. For example, the ``man
in hole'' story shape described in Section~\fref{sec:structure} could simply be described as a rearrangement of the
three-act structure:
...
Table's molecule model of tropes from Figure~\fref{fig:periodic-examples}: that
...
\emph{Trope B} occur. This is discussed further in Section~\fref{sec:branch-code}.
...
language (described in Chapter~\fref{cha:tropical}) for story authoring to the Oxford and London Interactive Fiction meetup group.
...
listed in Appendix~\fref{appendix:questionnaire}. The fact that all of the interactive fiction authors and games developers were
...
Chapter~\fref{cha:tropical}, where we describe the language in detail. In the same manner as a wiki, once a number of authors have contributed tropes,
...
section~\fref{sec:litrev-discussion}:
...
% In section~\fref{sec:pjexample}, we described the ``sausages'' scene from
...
% section of the next chapter (Section~\fref{sec:bridges}).
% \end{compactdesc}
...
chapter~\fref{cha:literature-review}, such as those using the Mimesis
...
language syntax in Section~\fref{sec:t-constrained}.
Section~\fref{sec:t-requirements} explores the use cases for our language, deriving
a set of requirements from them. Based upon these requirements, the features and
design of the language are described in Section~\fref{sec:language-design}.
The translation of TropICAL to InstAL code is demonstrated in Section~\fref{sec:t-codegen}, with samples of generated InstAL
institutions, Answer Set
generation (section~\fref{sec:t-asp}), constraints
(section~\fref{sec:t-constraints}), and output traces (section~\fref{sec:example-traces}). Finally, we show its extension for
the description of legal policies (section~\fref{sec:t-legal}). The chapter
concludes with a summary in Section~\fref{sec:tropical-summary}, describing how
some requirements from Section~\fref{sec:requirements} were met, while others are
addressed through the \emph{StoryBuilder} tool described in Chapter~\fref{cha:storybuilder}.
...
implemented in practice, see Section~\fref{sec:punchjudy} of
Chapter~\fref{cha:institutions}.
...
Based on the use cases above (section~\fref{sec:use-cases}), we can create the
following requirements:

\begin{enumerate}[R1.]
  \item\label{req:agents} The software must be able to integrate into a
    multi-agent framework such as Jason (Alice R\fref{req:alice1})
  \item\label{req:direct} The system must direct the behaviour of agents to fit
    a story description (Alice R\fref{req:alice2})
  \item\label{req:components} The user must be able to write and re-use
    existing story components (from a library) (Alice R\fref{req:alice3}, R\fref{req:alice5} and R\fref{req:alice6}, Bob R\fref{req:bob2})
  \item\label{req:sequences} The components must be able to express sequences of
    events (Alice R\fref{req:alice4}, Bob R\fref{req:bob1})
  \item\label{req:branches} The components must be able to express branching
    (diverging) events (Alice R\fref{req:alice4}, Bob R\fref{req:bob1})
  \item\label{req:subtropes} The user must have the ability to nest existing
    components inside new components (Bob R\fref{req:bob4})
  ...
  \item\label{req:norm} The story must tell the character agents what to do, but
    ...
    degree of unpredictability (Charlie R\fref{req:charlie1})
\end{enumerate}
...
\begin{itemize}
  \item Entity Declarations (Section~\fref{sec:dec-code})
  \item Sequences of Events (Section~\fref{sec:seq-code})
  \item Obligations (Section~\fref{sec:obl-code})
  \item Branching Events (Section~\fref{sec:branch-code})
  \item Subtropes (Section~\fref{sec:subtrope-code})
\end{itemize}
...
% definitions can be found in Appendix~\fref{appendix:tropes}.

This section begins by showing how the entities (roles, objects and places) of a
trope are defined by the author (Section~\fref{sec:dec-code}), then follows this with a description of how
these entities' actions are composed together to form sequences of events (Section~\fref{sec:seq-code}).
Normally, these events give the character entities \emph{permission} to perform
certain actions, so Section~\fref{sec:obl-code} shows how an author may
...
in Section~\fref{sec:branch-code}. Finally, the means for creating trope
abstractions by embedding existing tropes into new ones is demonstrated in
section~\fref{sec:subtrope-code}.
...
We do this to fulfil requirement~\fref{req:norm}, so that we \emph{guide}
the actions of character agents to fit our story, rather than regimenting their
behaviour. Section~\fref{sec:seq-code} describes how this is implemented when compiled. It is
possible to tell the agents what to do in a stronger fashion using
\emph{obligations}, which we describe in Section~\fref{sec:obl-code}.

Typically, tropes tend to consist
of a sequence of several events, as is stated in requirement R\fref{req:sequences}. So we can
...
actions with more control (as per requirement R\fref{req:direct}). The syntax for this type of action
...
event has occurred. Listing~\fref{lst:obl2} shows an example trope where the Hero
...
Section~\fref{sec:instal-obl} explains in detail how obligations, deadlines and
consequences are compiled to InstAL code.

\subsection{Branching Events}
\label{sec:branch-code}
As requirement R\fref{req:branches} states, a trope author may want to describe alternative possibilities in their story,
...
Requirement R\fref{req:subtropes} states that we need a mechanism for the
...
section~\fref{sec:combining-tropes} of Chapter~\fref{cha:storybuilder}.

For technical details of the parser implementation, including the EBNF grammar
of the TropICAL language, refer to Appendix~\fref{appendix:t-grammar}.
% Appendix~\fref{appendix:parse-tree} shows visualisations of the parse tree output for some
...
InstAL, with fully translated examples of InstAL code appearing in Appendix~\fref{appendix:instal}.
...
described in Section~\fref{sec:lit-insts}) for multi-agent systems. InstAL code
...
Cliffe's model specifies three types of \textbf{event}: \emph{external events} (or `observed events', $\mathcal{E}_{obs}$), \emph{institutional events} ($\mathcal{E}_{instevent}$) and \emph{violation events} ($\mathcal{E}_{viol}$). Examples of each are given in Figure~\fref{fig:events-pj}.
...
changes, or the triggering of story functions such as Propp's \emph{absentation} or \emph{interdiction} (described in Section~\fref{sec:formalisms}). 
...
described in terms of story tropes, appears in Section~\fref{sec:tropes-as-insts}.
...
descriptions. The StoryBuilder tool described in Chapter~\fref{cha:storybuilder}
...
simulation (Section~\fref{sec:freedom-example} gives an example of how this kind
...
\item \emph{Exogenous Event Declarations}: exogenous (external) events are
  ...
  (lines~\fref{lne:ex-start} -~\fref{lne:ex-end})
\item \emph{Violation Event Declarations}: events that are triggered when obligations
  are not fulfilled (line~\fref{lne:viol-event})
\item \emph{Institutional Event Declarations}: these are events that occur
  ...
  (lines~\fref{lne:instev-start} -~\fref{lne:instev-end})
\item \emph{Obligation Fluent Declarations}: obligation fluents have three
  ...
  (line~\fref{lne:obl-fluent}, obligation events are described further in Section~\fref{sec:instal-obl})
\item \emph{Norm / fluent initiation events}: this part of an institution
  ...
  (internal) events (lines~\fref{lne:init-start} -~\fref{lne:init-end}, described
  in Section~\fref{sec:anat-init})
\item \emph{Norm / fluent termination events}: this specifies which norms and
  fluents are terminated by institutional events (lines~\fref{lne:term-start}
  -~\fref{lne:term-end}, described in Section~\fref{sec:termination})
\item \emph{Generation events}: institutional events that are generated by
  exogenous (external) events (line~\fref{lne:gen-start} -~\fref{lne:gen-end},
  described in Section~\fref{sec:anat-gen})
\item \emph{Norms / fluents that hold initially in the institution}: this
  ...
  (lines~\fref{lne:initially-start} -~\fref{lne:initially-end}, described in Section~\fref{sec:anat-initial})
\end{itemize}
...
Listing~\fref{lst:macguffin-listing} shows a simplified ``MacGuffin'' trope in
...
Listing~\fref{lst:macguffin-hashmap}. In the listing, a hash map is contained
...
caption={Hash Map representation of the ``MacGuffin trope from Listing~\fref{lst:macguffin}''}]
{:label "MacGuffin",
...
line~\fref{lne:instev-start} of Listing~\fref{lst:instal-anatomy} is declared with four parameters of types
\emph{Agent, Agent, PlaceName, PlaceName}. When this institutional event is used
to initiate fluents on line~\fref{lne:init-start}, its parameters are input as
...
appears on lines~\fref{lne:init-vars} and~\fref{lne:init-end} of Listing~\fref{lst:instal-anatomy}.
...
the components of an InstAL institution as show above and in Listing~\fref{lst:instal-anatomy}.
...
As described previously in Section~\fref{sec:inst-events}, an institution
...
external events to occur. Returning to Listing~\fref{lst:seq4} above, the
...
Section~\fref{sec:inst-events}):

\begin{lstlisting}[showstringspaces=false, label={lst:instal-generates},
caption={InstAL code resulting from the compilation of Listing~\fref{lst:seq4}}]
find(R, T) generates
...
section~\fref{sec:event-phases}) below.
...
institution (described previously in Section~\fref{sec:trope-phases}). At the beginning of the trope, its state is simply \emph{active}.
...
section~\fref{sec:termination}). This initiation and termination process then
...
In the case of our example sequence of events from Listing~\fref{lst:seq4}, there
...
initiation code for Listing~\fref{lst:seq4}}]
...
those initiated in Listing~\fref{lst:initiates} is shown in
...
termination code for Listing~\fref{lst:seq4}}]
...
branches shown in Listing~\fref{lst:branch3}:
...
A fully compiled institution appears in Section~\fref{appendix:branch3} of
Appendix~\fref{appendix:instal}, which uses the following entity instances:
...
\emph{terminates} clauses. In Listing~\fref{lst:branch-init} below, we see that three possible events are given
...
branching trope in Listing~\fref{lst:branch3}}]
...
Returning to Listing~\fref{lst:obl1} of Section~\fref{sec:obl-code}, which
...
Line~\fref{line:obl1-oblev} of Listing~\fref{lst:obl1-init} shows the
...
\emph{intGo(hero, landOfAdventure)}. Listing~\fref{lst:obl1-gen} shows how the
...
to occur on line~\fref{line:obl1-oblev} of Listing~\fref{lst:obl1-init}, along with
the obliged institutional event itself.

\begin{lstlisting}[label={lst:obl1-fluent}, caption={Fluent declaration for the
obligation event in Listing~\fref{lst:obl1}}]
...
second (obligation) event of the trope in Listing~\fref{lst:obl1}}, escapechar=|]
...
The full InstAL institution that this generates appears in Listing~\fref{lst:obl1-instal} of appendix~\fref{appendix:instal}.

\subsubsection{Obligations with Deadlines and Consequences}
The trope shown in Listing~\fref{lst:obl2-trope} describes a situation where an
...
Listing~\fref{lst:obl2-fluent} shows the fluent declaration of the obligation
event in Listing~\fref{lst:obl2-trope}. The deadline event \emph{intKill(villain,
  mentor)} is an institutional event, and so is generated by the
\emph{kill(villain, mentor)} external event on
lines~\fref{line:obl2-deadline1} to~\fref{line:obl2-deadline2} of
listing~\fref{lst:obl2-gen}. Listing~\fref{lst:obl2-init} shows the obligation
...
the permission of the villain to kill the hero. This is shown in the code of Listing~\fref{lst:obl2-trope}.
...
both a deadline and a consequence, translated from the trope in Listing~\fref{lst:obl2-trope}}]
...
violation event of the trope in Listing~\fref{lst:obl2-trope}}]
...
trope in Listing~\fref{lst:obl2-trope}}, escapechar=|]
go(R, U) generates
    intGo(R,U) if
        role(R, hero),
        place(U, landOfAdventure);
...
In Section~\fref{sec:subtrope-code}, we describe how \emph{subtropes} are
...
Listing~\fref{lst:bridge-inst} shows the bridge that links the
...
line~\fref{line:cross1}. This line only states that this is a fluent that is to
be initiated in the sink institution, from the source institution.

Lines~\fref{line:crossperm1} to~\fref{line:crossperm2} describe the actual
...
mechanism is used on lines~\fref{line:phase1} and~\fref{line:phase2} to declare
...
Listing~\fref{lst:bridge-source} shows the generation event corresponding to the
...
in Listing~\fref{lst:bridge-inst}.

Full listings for the generated InstAL code for these tropes, along with several
other examples, appear in Appendix~\fref{appendix:instal}.
...
Listing~\fref{lst:constraints} shows the AnsProlog rules used to constrain the
answer sets produced by the solver, so that they only consist of events that are
relevant to our stories. Line~\fref{line:viol} specifies that events are only
valid if they are not violation or null events. Line~\fref{line:inst} filters
for events that occur inside institutions. Line~\fref{line:valids} ignores
any answer sets where a valid event occurs after a sequence of non-valid events.
Lines~\fref{line:dead} and~\fref{line:dead2} ignore our dummy deadline events for
obligations where no deadline is specified. Finally, lines~\fref{line:null}
and~\fref{line:null2} remove any \emph{null} events at all from the answer sets.
...
such as the one in Listing~\fref{lst:constraints}, we can tell the answer set
...
answer sets, refer to the diagrams in Section~\fref{sec:trope-visualisation}.
...
Appendix~\fref{appendix:evil-empire}. The traces listed below are solved by
...
Listing~\fref{lst:evil-trace} shows one of the four traces from the solver
...
Figure~\fref{fig:evil-trace} shows a visualisation of the events that occur in
the answer set in Listing~\fref{lst:evil-trace}.
...
\caption{Visualisation of the ``Evil Empire'' example trace in Listing~\fref{lst:evil-trace}}\label{fig:evil-trace}
\end{figure}

The events that appear at the start of the listing (starting with the
\emph{holdsat} predicate) on line~\fref{lne:as-holdsat} describe the \emph{fluents} that hold for a certain
...
institutional events with the institution name, which is why the \emph{chase} (line~\fref{lne:as-ex1}),
\emph{capture} (line~\fref{lne:as-ex2}) and \emph{escape} (line~\fref{lne:as-ex3}) events all trigger the \emph{intEvilEmpire}
institutional event on lines~\fref{lne:as-int1}, \fref{lne:as-int2} and~\fref{lne:as-int3} of Listing~\fref{lst:evil-trace}.
...
listing~\fref{lst:hero-trace} as an example, and a visualisation appears in Figure~\fref{fig:hero-trace}.
...
\caption{Visualisation of the ``Hero's Journey'' example trace in Listing~\fref{lst:hero-trace}}\label{fig:hero-trace}
\end{figure}
...
together is shown in Listing~\fref{lst:hero-evil-trace} in
Appendix~\fref{appendix:full-trace}. A visualisation is shown in
Figure~\fref{fig:hjee-trace} in the same appendix.
...
reader refer to the visualisations produced by the StoryBuilder tool in Section~\fref{sec:trope-visualisation}.


\section{Summary}
\label{sec:tropical-summary}
Returning to the requirements listed in Section~\fref{sec:requirements} (listed
...
\begin{enumerate}[R1.]
  \item \textbf{The software must be able to integrate into a
    multi-agent framework such as Jason}: The ``Punch and Judy'' example in Section~\fref{sec:punchjudy} describes
...
obligation with a strict consequence for non-compliance.
  ...
  \item \textbf{The components must be able to express sequences of events}: Section~\fref{sec:seq-code} shows how sequences of events can be described in
TropICAL.
  \item \textbf{The components must be able to express branching
    (diverging) events}: Section~\fref{sec:branch-code} describes the
  ...
  language. Section~\fref{sec:subtrope-code} describes the
...
mentioned above (numbers~\fref{req:agents}, \fref{req:direct},
\fref{req:sequences}, \fref{req:branches}, \fref{req:subtropes} and
\fref{req:norm}). However, there are two requirements that cannot be fulfilled
through a text-based programming paradigm alone: selecting pre-existing story
components from a library (requirement~\fref{req:components}), and visualising
the branches of the story as it is modified (requirement~\fref{req:vis}). These
...
visualisation of tropes. The next chapter (Chapter~\fref{cha:storybuilder})
...
R\fref{req:components} and R\fref{req:vis} in the requirements listed in Section~\fref{sec:requirements}:
...
To address requirement~\fref{req:vis}, we parse the answer set outputs (traces)
of our answer set solver (as described in Section~\fref{sec:trope-visualisation}) to create a tree
...
Figure~\fref{fig:sb-start} shows what the user sees when they first start the
...
is blank in this initial screenshot). Figure~\fref{fig:sb-edit-ann} shows the
same screen as Figure~\fref{fig:sb-start}, with the addition of annotations that
...
Figure~\fref{fig:sb-new-ann} shows the five steps that a user must take in order
to create a trope:
...
which we refer to as \emph{answer sets} or \emph{traces} in Section~\fref{sec:t-asp}, are lists of events that are possible in one run
...
are explained further in Section~\fref{sec:example-traces}.
...
Figure~\fref{fig:sb-arrange} shows the view that the user sees when the
...
on the selected tropes. Figure~\fref{fig:sb-combine-ann} shows the steps that a
...
shown in Figure~\fref{fig:sb-combine-ann}, events that follow the \emph{Example
...
\caption{Two consecutive events (from Listing~\fref{lst:seq2})}\label{fig:sb-seq1}
\end{figure}

\begin{figure}[!t]
\centerline{\includegraphics[width=\textwidth]{storybuilder-seq2.png}}
\caption{A sequence of events (from Listing~\fref{lst:seq4})}\label{fig:sb-seq2}
\end{figure}

\subsection{Usage Examples}

The figures in this section show how the simple trope examples listed in Sections~\fref{sec:seq-code}
...
\caption{Two branches (from Listing~\fref{lst:branch1})}\label{fig:sb-branch1}
\end{figure}

\begin{figure}[!t]
\centerline{\includegraphics[width=\textwidth]{storybuilder-branch2.png}}
\caption{Five branches (from Listing~\fref{lst:branch2})}\label{fig:sb-branch2}
\end{figure}
...
from Listing~\fref{lst:seq2} as input. Figure~\fref{fig:sb-seq2} uses the longer
sequence of five events from Listing~\fref{lst:seq4} as its input.

\emph{StoryBuilder}'s visualisation of branches are demonstrated in the figures
that follow. Figure~\fref{fig:sb-branch1} shows the simple two-branch trope of
listing~\fref{lst:branch1}. Figure~\fref{fig:sb-branch2} shows the visualisation
...
sequences and branches from Listing~\fref{lst:branch3} is shown in Figure~\fref{fig:sb-branch3}.

\begin{figure}[!t]
\centerline{\includegraphics[width=\textwidth]{storybuilder-branch3.png}}
\caption{A combination of branches and sequences (from Listing~\fref{lst:branch3})}\label{fig:sb-branch3}
\end{figure}


\begin{figure}[!t]
\centerline{\includegraphics[width=\textwidth]{storybuilder-subtrope1.png}}
\caption{Subtrope to be embedded (from Listing~\fref{lst:subtrope1})}\label{fig:sb-subtrope1}
\end{figure}


The final two figures in this section demonstrate the use of \emph{StoryBuilder}
for embedding subtropes inside of other tropes. Figure~\fref{fig:sb-subtrope1} is
the \emph{StoryBuilder} visualisation that corresponds to the subtrope that is
to be embedded (described in Listing~\fref{lst:subtrope1}). The actual embedding
of this subtrope, as described in Listing~\fref{lst:subtrope2}, and its resulting visualisation are shown in Figure~\fref{fig:sb-subtrope2}.

\begin{figure}[!t]
\centerline{\includegraphics[width=\textwidth]{storybuilder-subtrope2.png}}
\caption{Trope containing a subtrope (from Listing~\fref{lst:subtrope2})}\label{fig:sb-subtrope2}
\end{figure}
...
Listing~\fref{lst:audience-participation} shows a basic description of the events
...
Figure~\fref{fig:audience-participation} shows how this trope appears when it is
...
the user interface, described in Section~\fref{sec:combining-tropes} below.

Listing~\fref{lst:babysitting-trope} shows a trope to describe the
...
suitable for younger audiences. The StoryBuilder visualisation appears in Figure~\fref{fig:babysitting-trope}.
...
Listing~\fref{lst:sausages-trope} shows the TropICAL definition of the
...
them below. Its StoryBuilder visualisation appears in Figure~\fref{fig:crocodile-sausages}.
...
``Chase Scene'' trope of Listing~\fref{list:chase-trope}.
...
repeating ourselves. Listing~\fref{list:babysitting-trope-subtrope} shows how the
``Babysitting'' trope would be rewritten by incorporating the ``Absentation''
and ``Chase Scene'' tropes as subtropes.
Listing~\fref{list:sausages-trope-subtrope} does the same for the ``Sausages'' trope.
...
output of the answer set solver. Figure~\fref{fig:combining-tropes} shows the
...
\added{Further discussion on trope combination appears in Section~\fref{sec:trope-combination-exp}.}{2.3.3: Forward ref to discussion on
...
addresses requirements R\fref{req:components} and R\fref{req:vis} in the
requirements shown in section~\fref{sec:requirements}:
...
StoryBuilder fulfills requirement R\fref{req:components} by storing every trope
that the author creates in a database, so that they may be reused and amended if
needed. Requirement R\fref{req:vis} is met through StoryBuilder's use of a tree
...
Section~\fref{sec:litrev-discussion} of the literature review identifies the following issues with current approaches to interactive storytelling:
...
Issue~\fref{iss:freedom} from the literature review. An institution is a
...
Using a social institution as a story model addresses Issue~\fref{iss:freedom} by
...
existing narrative formalism (Section~\fref{sec:stories-logic}). The same scene
is then described as a set of norms in an institution in
Section~\fref{sec:norms-and-institutions}. Section~\fref{sec:punchjudy} describes
...
summary in Section~\fref{sec:inst-summary}.
...
\changed{As we describe in Section~\fref{sec:lit-insts} of the literature review,
...
As Section~\fref{sec:instal-intro} explains, an institution describes the
...
figures~\fref{fig:events} and~\fref{fig:term}, refer to
Section~\fref{sec:instal-intro} and~\citep{cliffe2007specifying}.

Using the same notation introduced in Section~\fref{sec:instal-intro}, Figure~\fref{fig:events} lists some external ($\mathcal{E}_{external}$) and institutional (internal, $\mathcal{E}_{internal}$) events for the \emph{Hero's Journey} trope. A wide range of external events such as \emph{go}, \emph{meet}, \emph{kill}, \emph{escape} generate the \emph{intHerosJourney} internal event, but only if the external event meets certain criteria. These criteria could be whether or not an agent fulfils a certain role, for example. Figure~\fref{fig:gen-hero} shows examples of such internal event generation ($\mathcal{G}$). In the first example (rule~\fref{eq:tatooine}), the \emph{intHerosJourney} event is generated when Luke Skywalker goes to Tatooine, but only if Luke has the role of \emph{hero}, and Tatooine's location is \emph{home}.
Figure~\fref{fig:init-hero} shows how internal events initiate
...
only shows the first two events of the trope (the \emph{phase} keywords are explained further in Section~\fref{sec:trope-phases}). Rule~\fref{eq:phaseb} shows how the \emph{intHerosJourney} internal event initiates the hero's permission to kill the villain, an obligation for the hero to go to the Land of Adventure before the villain kills the victim, and the next phase (phase C) of the \emph{Hero's Journey} trope. These fluents are only initiated if the \emph{intHerosJourney} internal event happens while the trope is in phase B (when \emph{phase(herosJourney, phaseB)} holds), however.
Fluent termination ($\mathcal{C^{\downarrow}}$) works in a similar manner to initiation, with permissions and obligations for previous trope phases being terminated once the next phase of a trope has been entered. Examples for the first two phases of the \emph{Hero's Journey} trope are shown in Figure~\fref{fig:term-hero}.
...
In Section~\fref{sec:pjexample}, we describe the \emph{sausages} scene of Punch
...
section~\fref{sec:pjexample-insts}, we create an institution out of the scene we
...
section~\fref{sec:pjexample} in terms of tropes, using this one piece of the
...
and so is discussed further only in the ``future work'' Section~\fref{sec:future}.
...
the \emph{or} operator (Section~\fref{sec:branch-code}) allowing the creation of more flexible tropes
...
Figure~\fref{fig:pj-phase-inits} shows how a short version of the \emph{Dont Touch
...
Figure~\fref{fig:pj-phase-terms}. The left side of Rule~\fref{eq:phase-inactive}
...
to the right of the arrow. In the case of Rule~\fref{eq:phase-inactive}, the \emph{intDontTouchItYouIdiot}
...
($\text{perm}\mathtt{(tellprotect(owner, idiot, item))}$). Rule~\fref{eq:phase-a}
...
\emph{dontTouchItYouIdiot} trope is in \emph{Phase A}. Rule~\fref{eq:phase-done}
simply sets the trope's phase to \emph{done} if it is in \emph{Phase B}, without
initiating any norms.

The termination rules in Figure~\fref{fig:pj-phase-terms} simply terminate any
phase fluents that hold (represented by the empty set in
Rules~\fref{eq:phase-inactive-term}, \fref{eq:phase-a-term}
and~\fref{eq:phase-done-term}), so that the new phase fluent replaces its predecessor.
...
institution described in Appendix~\fref{appendix:sausages-full}.
...
description of how this might work appears in Section~\fref{sec:freedom-example}.
...
total of 27 distinct emotional states, shown in Figure~\fref{fig:vad}.
...
states. Section~\fref{sec:pj-fullspec} gives a further example of an agent emotional
...
Valence, arousal and dominance values can take values of -1 (low), 0 (medium) or 1 (high). Listing~\fref{lst:emotions} shows the emotional belief rules for an agent with medium dominance (a dominance level of 0). Note that an agent maintains beliefs about both its current emotion label (such as sleepy or happy) and the separate valence, arousal and dominance values at the same time.  Similar sets of rules handle the belief emotion for the other dominance levels.  %We address the matter of how external stimuli affect dominance in section~\fref{sec:norms as percepts}.\jnote{still needs doing}

Every time an emotional variable (valence, arousal, or dominance) changes, an agent's emotion is changed according to the rules in Listing~\fref{lst:emotions}. While an agent's valence, arousal and dominance belief values affect the way it makes decisions internally, the results of combinations of these values (sleepy, happy, etc) are broadcast as external actions. The reason for this is that an agent's emotional state may affect the way in which the character is animated: changing the speed at which they move or turning their smile into a frown, for example. For this reason, whenever an emotional change takes place, the new emotion is published as an external action of the agent so that observing entities may perceive it. The Bath sensor framework described in Section~\fref{sec:arch} provides the means for this evidence of the agent's internal state change to be received by the animation system and reflected accordingly in the display.
...
Listing~\fref{lst:response} shows the AgentSpeak rules describing how an agent's
...
Listing~\fref{lst:response}, the $\texttt{+!changeMood}$ plan on
lines~\fref{lne:changemood-start} to~\fref{lne:changemood-end} updates the agent's
...
(Listing~\fref{lst:response}, lines~\fref{lne:response1-start} - \fref{lne:response1-end}
and~\fref{lne:response2-start} - \fref{lne:response2-end}) raise or lower an agent's valence and dominance levels depending on whether the agent perceives a ``boo'' or ``cheer'' response from the audience.
...
\emph{absentation} function, shown in Listing~\fref{lst:absentation}. InstAL rules like those shown above and in Listing~\fref{lst:absentation} are compiled into AnsProlog, then we use the \emph{clingo} answer set solver~\citep{gebser2011potassco} to ground the program, and `solve' queries by finding all permissions and obligations that apply to any agents, given a sequence of events as the query input. The agents' percepts are then updated with their permitted and obliged actions from that time instant onwards.  Thus, the institutional model acts as a social narrative sensor, interpreting actors' actions in the context of the combination of the concrete narrative and the abstract story moves which detach (instantiate) the norms that guide the actors in the direction of the conclusion of the story arc.
...
\emph{sausages} is described in Listing~\fref{lst:trace}. The `pj' in the trace
...
institution, as determined by the generates relation. Listing~\fref{lst:asp}
...
certain instants when they have some effect on the scene, although in practice, all role fluents hold throughout the scene. Figure~\fref{fig:tracevis} shows a visualisation of the answer set for the trace in Listing~\fref{lst:trace}.
...
of limited agent autonomy in existing systems (Issue~\fref{iss:freedom} from
Section~\fref{sec:litrev-discussion}), the advantages of this approach are:
...
comes with current approaches. For this reason, Chapter~\fref{cha:tropes} explains how
...
natural-language system for defining story components, which we describe in Chapter~\fref{cha:tropical}.
...
Section~\fref{sec:sb-eval}. Next, a validation of the TropICAL language is
described through a complete description of a Punch and Judy story in
Section~\fref{sec:pj-fullspec}. A discussion on how tropes are combined together
is in Section~\fref{sec:trope-combination-exp}, and the chapter concludes with an
examination of the extent to which ``character freedom'' has been achieved, and
the potential benefits of this, in Section~\fref{sec:freedom-example}.
...
by~\citet{clarke2014thematic}, and in Section~\fref{sec:sb-analysis}).
...
analysis is not designed to evaluate requirements~\fref{req:agents},
\fref{req:direct} and~\fref{req:norm} from Section~\fref{sec:requirements}, only
...
Section~\fref{sec:requirements}, and so are used as the basis of the questions
asked throughout the thematic analysis:

\begin{itemize}
  \item How easy or difficult did you find the language to learn? (Requirement~\fref{req:tropes-natural})
  \item Could you create the story you wanted by combining tropes together? (Requirement~\fref{req:tropes-reusable})
  \item Were you able to create the abstractions you wanted using subtropes? (Requirement~\fref{req:tropes-abstractions})
  \item Did the visualisations assist you in the creation of tropes and stories? (Requirement~\fref{req:tropes-vis})
\end{itemize}
...
given in Section~\fref{sec:user-tasks}.
...
Appendix~\fref{appendix:transcripts}.
...
tropes, testing requirement R\fref{req:components}.
...
This also tests requirements R\fref{req:components} and R\fref{req:sequences}.
...
testing requirement R\fref{req:branches}.
...
tests requirement R\fref{req:vis}.
...
requirement R\fref{req:subtropes}.
...
This task tests requirements R\fref{req:components} to R\fref{req:vis} by asking
...
\begin{itemize}
  ...
  \item Branches cannot be extended beyond one event (as described by indenting
    ...
    Listing~\fref{lst:branch4} of Section~\fref{sec:branch-code}), again due to time
  ...
  \item Branches cannot be cut off (by writing ``Then the story ends'', shown in
    Listing~\fref{lst:branch5}), and so
...
Appendix~\fref{appendix:transcripts}. These themes fall into four categories:
...
in TropICAL is described in Section~\fref{sec:entity-instances}. The user was able to select named characters to fulfil
...
Section~\fref{sec:structure}: ``Things are good, then things are bad, then things
...
be interpreted (as described in Section~\fref{sec:seq-code}). Once users were told this fact, they had no problem choosing
...
As explained above (Section~\fref{sec:limitations}), the \emph{clingo} answer-set solver that StoryBuilder uses
...
In section~\fref{sec:punchjudy-tropes}, we describe one scene from Punch and Judy
...
to talk to the audience. Listing~\fref{lst:pjfull-narrator} shows how this trope
...
Listing~\fref{lst:pjfull-participation} shows how this trope could be translated
...
Section~\fref{sec:dont-touch-inst}, this is where Judy tells Punch to look after
the baby.
...
previous section (Section~\fref{sec:full-introduction}).
...
Mischief'', and its TropICAL interpretation appears in Listing~\fref{lst:pjfull-mischief}
...
wife fight with each other, as described in the TropICAL code in Listing~\fref{lst:pjfull-domesticabuse}.
...
The slapstick trope can be defined in TropICAL with the code shown in Listing~\fref{lst:pjfull-slapstick}.
...
The ``Chase Fight'' trope already appears in the previous scene where Punch kills his baby and wife (Listing~\fref{lst:pjfull-chasefight}), 
...
The translation of the ``Mini-boss'' trope into TropICAL code is shown in Listing~\fref{lst:pjfull-miniboss}.
...
Listing~\fref{lst:pjfull-mishandling} shows a trope named ``Mishandling of an Item'' that describes the various
ways in which an item can be mishandled. Listing~\fref{lst:pjfull-mischief2} is
...
The story begins with the ``Introduction'' scene shown in Listing~\fref{lst:pjfull-intro1}, with the character of Joey
...
Listing~\fref{lst:pjfull-fullstory}). This is because the trope appears in every
...
Listing~\fref{lst:pjfull-baby1} shows the definition of the ``Punch, Judy and the
...
which is shown in Listing~\fref{lst:pjfull-police1}. In this scene, Punch has the
...
sausages. Listing~\fref{lst:pjfull-croc1} shows the TropICAL code for the
...
Listing~\fref{lst:pjfull-devil1}. This scene contains exactly the same tropes as
...
Listing~\fref{lst:pjfull-fullstory} shows the TropICAL definition of the entire
...
evaluated prototype described in Chapter~\fref{cha:evaluation} does not include
...
Section~\fref{sec:asl-character-roles} of the appendix, with the code for each
character agent appearing in Section~\fref{sec:asl-character-agents}. A worked
...
Listing~\fref{lst:pj-emotion} shows the shared plan library for agent emotions,
...
situation is described in Section~\fref{sec:freedom-example}.
...
Listing~\fref{lst:pjfull-idiotagent} shows the AgentSpeak plans for this role.
...
Listing~\fref{lst:pjfull-punchagent}. As well as inheriting the common code for
...
Section~\fref{sec:asl-character-roles} of the appendix.
...
\added{
  ...
  Section~\fref{sec:subtrope-code} describes the use of subtropes in TropICAL,
  ...
  Section~\fref{sec:bridges}. This section examines how our system allows
  ...
  (shown in Figure~\fref{fig:sb-combine-ann}). Alternatively, if not using StoryBuilder,
  the following code in Listing~\fref{lst:simultaneous-tropes} can be passed to
...
\end{lstlisting}
  ...
  Figure~\fref{fig:dot-combine1} shows an example where the first events of each
  ...
  permitted or obliged to occur, as shown in Figure~\fref{fig:dot-combine2}. Now
  ...
  Figure~\fref{fig:dot-combine3} shows the situation after two choices from the Hero.
...
in Section~\fref{sec:croc-sausages-scene}), where Punch has been given the
...
``Thief'' character roles, such as the one shown in Listing~\fref{lst:thief-trope}.
...
Listing~\fref{lst:pjfull-directoragent}. The director agent could even make use
...
Section~\fref{sec:planner-systems}, noting the similarities and comparative
benefits and drawbacks our approach provides. Next follows a discussion in
Section~\fref{sec:discussion-freedom} of the extent of ``character freedom'' our
...
Section~\fref{sec:sb-eval}, by comparing it with two
planner-based approaches mentioned in Section~\fref{sec:planner-systems} of the literature review: Young's
...
demonstrated in the evaluation in Section~\fref{sec:sb-eval}, is to allow non-programmer story authors to create
...
allow. Section~\fref{sec:put-together-pj} describes how such a system could be
...
in Section~\fref{sec:future-agent}.
...
Section~\fref{sec:normative-intro} of the introduction. One of the benefits of
...
Section~\fref{sec:pj-fullspec} would achieve it.

\subsection{Prototype Implementation}
The evaluation in Chapter~\fref{cha:evaluation} examined the suitability of
...
in Listing~\fref{lst:constraints}. These violations are important to keep track
of, as they tell us that an agent is trying to break away from the permitted
course of the story.

\subsection{Proposed Full Implementation}
Section~\fref{sec:pj-fullspec} describes a potential implementation of TropICAL
...
This section refers back to Section~\fref{sec:narrative-types} of the literature
...
as we propose in Section~\fref{sec:topicscript}, our system could eventually meet
this definition.
...
The listing examples from Section~\fref{sec:language-design} are used at first to
isolate individual TropICAL features (Listings~\fref{lst:ex-seq}
to~\fref{lst:ex-subtrope}), with Listings~\fref{lst:ex-sim} to~\fref{lst:ex-three} showing the combination of several tropes together.
...
The trope in Listing~\fref{lst:ex-seq} is a basic sequence, five events long, one
event after another.
...
in Listing~\fref{lst:ex-subtrope1}, which is included at the end of both active
...
\begin{enumerate}
\item Chapter~\fref{cha:institutions} introduces the idea of using social
...
\item In Chapter~\fref{cha:tropes}, we describe the use of \emph{story tropes} as a method of creating story components in
...
\item Chapter~\fref{cha:tropical} introduces \emph{TropICAL}, our controlled natural
...
\item Chapter~\fref{cha:storybuilder} addresses the limitations of the
...
allowing them to break away from the story in some situations. The thematic analysis of \emph{StoryBuilder} conducted in Section~\fref{sec:sb-eval} reveals that
...
proved effective during the study described in Section~\fref{sec:sb-eval}, some limitations and possibilities for
expansion emerged during the evaluation of the system. These are addressed in
Section~\fref{sec:future-adress}. In the first Section of this chapter, however,
...
described by Vonnegut in Section~\fref{sec:structure}, or the ``Hero's Journey''
...
``Freytag's Pyramid'' from Section~\fref{sec:abstraction}, for example).
...
Section~\fref{sec:story-gen}. Rather than being used for dynamic, interactive
...
%   \emph{StoryBuilder} systems (see Section~\fref{sec:future-address}).
% \item\label{opp:agents} Create methods to allow non-programmer authors to author the
%     character agents that appear in their stories (see Section~\fref{sec:future-agent}).
%   \item\label{opp:domains} Explore how \emph{TropICAL} could be applied to domains outside of
%     interactive storytelling (see Section~\fref{sec:future-domains}).
% \end{enumerate}

\subsection{Addressing Limitations in \emph{TropICAL} and StoryBuilder}
\label{sec:future-address}

During the study described in Section~\fref{sec:sb-eval}, users highlighted the
...
Limitation~\fref{lim:five} can be addressed by increasing the number of time steps
...
To solve Limitation~\fref{lim:branch}, Fluents could be
...
Limitation~\fref{lim:sub} is an issue in the implementation of the prototype
used for the study, and would only require time to be spent debugging the
compiler to produce the correct output.

Limitation~\fref{lim:error} could be fixed by referring back to the line of
...
Improvement~\fref{im:three} would require extending the parser to deal with
...
Improvement~\fref{im:sem} would not be trivial to implement. One possible
...
Improvement~\fref{im:ent} would be implemented by extending the parser to allow
other types of entities to be declared in a trope definition.

Improvement~\fref{im:struc} is an improvement that would require implementation
...
Improvement~\fref{im:cond} would require extending the parser to look for an
...
Section~\fref{sec:punchjudy}), these lines of dialogue would have to be
...
model described in Section~\fref{sec:emotion}, then their character role could
...
answer set solver, in the same way in which we check and generate stories in Section~\fref{sec:t-asp}.
...
listings~\fref{lst:ebnf1} and~\fref{lst:ebnf2}. Listing~\fref{lst:ebnf1} describes
the syntax for the entity declarations at the top of each trope definition file,
as described in Section~\fref{sec:dec-code}. Listing~\fref{lst:ebnf2} shows the
...
\caption{Visualisation of trace for both the ``Evil Empire'' and ``Hero's Journey'' in Appendix~\fref{appendix:full-trace}}\label{fig:hjee-trace}
\end{figure}
...
% Line~\fref{lne:a-feature1} of participant A.
