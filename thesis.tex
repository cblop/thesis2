...
expansion emerged during the study described in Section~\ref{sec:sb-eval}.
...
\item\label{opp:limitations} Address the limitations of the current \emph{TropICAL} and
  \emph{StoryBuilder} systems
\item\label{opp:agents} Create methods to allow non-programmer authors to author the
    character agents that appear in their stories
  \item\label{opp:domains} Explore how \emph{TropICAL} could be applied to domains outside of
    interactive storytelling
\end{enumerate}

Section~\ref{sec:future-address} describes how to address
Opportunity~\ref{opp:limitations} by fixing the current limitations of TropICAL
and StoryBuilder. Section~\ref{sec:future-agent}. Section~\ref{sec:future-domains}.
...
that the answer set solver solves for, but the problem of it slowing down for multiple
tropes will remain. More work is needed in the refining the generated
institutions, along with the constraint rules that are input into the solver, in
order to make sure less answer sets are generated as a whole.

Limitation~\ref{lim:branch} is more difficult to solve. Fluents will have to be
...
story, for example: $\mathtt{branchPhase(branch1, phaseA)}$

Limitation~\ref{lim:sub} 

\\Users also suggested some improvements that could be made to the system:

\begin{enumerate}[{I}1.]
  \item\label{im:three} \textbf{Event with Three Entities}
  \item\label{im:sem} \textbf{Event Semantics}
  \item\label{im:ent} \textbf{Other Types of Entities}
  \item\label{im:struc} \textbf{Better Control Over Story Structure}
  \item\label{im:cond} \textbf{Conditionals}
\end{enumerate}
...
norms) of the agents''' behaviour.

Our current implementation of the system integrates with the JASON multi-agent
...
functional). For this reason, it would be even more challenging for
...
without spending a lot of extra time learning a new programming language.
...
\footnote{\url{https://www.inklestudios.com/ink/}, accessed 20171025.}, which is
