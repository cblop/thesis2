...
other narrative models described in the \emph{Related Work} section (Chapter~\ref{cha:literature-review}).
...
\section{Intelligent Agents as Story Characters}
\label{sec:character-agents}
...
system.\mnote{Qualified this by emphasising \emph{agent-based} and explaining
  how Inform lacks in that regard}
...
narrative.\mnote{Added explanation of agent-oriented nature of TropICAL}
...
Our TropICAL language design is as follows:\mnote{Changed to \emph{design},
  rather than \emph{use}}
...
documentation for an overview of the language~\citep{ace-nutshell}.\mnote{Added
  footnote with URL of documentation page}
...
paradigm is successful.\mnote{Added footnote with evidence for Inform 7's popularity}
...
story. \mnote{Added reference back to section where this is introduced} These agents, through an architecture such as BDI (Belief, Desire,
...
story. \mnote{Made it clearer that control is not tight} TropICAL is designed to be used with a system where the character agents have
...
chapter~\ref{cha:agents}.\mnote{May need to change this ref when chapter moved}
...
\end{enumerate}\mnote{Numbered requirements}
...
\end{enumerate}\mnote{Referred to user's requirements}
...
same type, these entity instances can be declared separately. \mnote{Removed
  technical details} An example with multiple Heroes, Swords and Evil Lairs would be:
...
If these separate instances are not declared, \mnote{removed reference to compiler} then each entity has just
...
\mnote{Removed implementation details and rephrased}
...
compiled code will be the same.\mnote{Added reason why}
...
that a trope needs to be subdivided. \mnote{Changed ``code'' to ``trope''} We can achieve this by
...
with any other kind of statement. \mnote{Added reason for using quotation marks} An example of putting this trope at the end
...
% INITIALLY -----------
initially~\label{lne:init-start}~
    ...
    role(obiWan, mentor),
    place(japan, landOfAdventure);~\mylabel{t9}\label{lne:init-end}~
...
(events that occur when an obligation is violated in an institution). \mnote{Added reference back to inst section} The institution watches for events to happen as \emph{external} events in the
...
Section~\ref{sec:inst-events}):\mnote{Explained int prefix for institutional events}
...
\begin{itemize}
  ...
  \item Branches cannot be extended beyond one event, again due to time
  ...
  \item Branches cannot be cut off (by writing ``Then the story ends''), and so
  ...
  \item The tropes must be event-based. This means that updating the state of
    ...
    code generation due to 
  \item Though the parser can translate obligations, deadlines and violation
    events from TropICAL to InstAL and ASP, the participants were not asked to
...
This feature is very possible to implement. In fact, it was featured in an early
...
\begin{itemize}
\item Participant A, line~\lineref{lne:feature1a}
\item Participant D, line~\lineref{lne:feature1d}
\end{itemize}
...
\begin{itemize}
\item Participant A, lines~\lineref{lne:feature2a},~\lineref{lne:feature2a2}~\lineref{lne:feature2a3}
\item Participant B, line~\lineref{lne:feature2b}
\item Participant D, line~\lineref{lne:feature2d}
\item Participant F, line~\lineref{lne:feature2f}
\end{itemize}
...
of event would be more complex than most users need.

  This theme appears in these lines of the transcript:

\begin{itemize}
\item Participant A, line~\lineref{lne:feature3a}
\item Participant D, line~\lineref{lne:feature3d}
\item Participant G, line~\lineref{lne:feature3g}
\end{itemize}
...
line~\lineref{lne:feature4a}) wrote a series of events where one character is killed, such as ``The Villain kills the Hero'',
...
\begin{itemize}
\item Participant A, line~\lineref{lne:feature4a}
\item Participant D, lines~\lineref{lne:feature4d},~\lineref{lne:feature4d2}
\item Participant H, lines~\lineref{lne:feature4h},~\lineref{lne:feature4h2}
\end{itemize}
...
\begin{itemize}
\item Participant B, line~\lineref{lne:feature5b}
\item Participant F, line~\lineref{lne:feature5f}
\end{itemize}
...
\item \textbf{Subject must be an agent}
  ...
  is an unavoidable constraint due to the fact that TropICAL describes social
  norms. Unless we allow objects to be simulated by agents in addition to roles,
  it would not make sense to give them permission to carry out actions.

  
This theme appears in these lines of the transcript:

\begin{itemize}
\item Participant B, line~\lineref{lne:feature6b}
\item Participant D, lines~\lineref{lne:feature6d},~\lineref{lne:feature6d2}
\end{itemize}
...
kind of climactic scene happens in an abstract way.

This theme appears in these lines of the transcript:

\begin{itemize}
\item Participant C, line~\lineref{lne:feature7c}
\item Participant F, line~\lineref{lne:feature7f}
\item Participant G, line~\lineref{lne:feature7g}
\end{itemize}
...
events, add an extra complexity to the language that was not needed during the
...
\begin{itemize}
\item Participant D, line~\lineref{lne:feature8d}
\item Participant H, line~\lineref{lne:feature8h}
\end{itemize}
\end{itemize}
...
\begin{itemize}
\item Participant A, line~\lineref{lne:syntax1a}
\item Participant D, lines~\lineref{lne:syntax1d},~\lineref{lne:syntax1d2}
\end{itemize}
...
is allowed by the parser, due to the fact that the verbs are fed into WordNet to
be interpreted. Once users were told this fact, they had no problem choosing
their own verbs to use in their tropes.

This theme appears in these lines of the transcript:

\begin{itemize}
\item Participant A, line~\lineref{lne:syntax2a}
\item Participant B, line~\lineref{lne:syntax2b}
\item Participant F, lines~\lineref{lne:syntax2f}
\item Participant G, lines~\lineref{lne:syntax2g}
\end{itemize}
...
\begin{itemize}
\item Participant A, line~\lineref{lne:syntax3a}
\item Participant B, lines~\lineref{lne:syntax3b},~\lineref{lne:syntax3b2}
\item Participant C, line~\lineref{lne:syntax3c}
\item Participant D, line~\lineref{lne:syntax3d}
\item Participant E, lines~\lineref{lne:syntax3e},~\lineref{lne:syntax3e2}
\item Participant F, line~\lineref{lne:syntax3f}
\item Participant H, line~\lineref{lne:syntax3h}
\end{itemize}
...
\begin{itemize}
\item Participant A, line~\lineref{lne:syntax4a}
\item Participant C, line~\lineref{lne:syntax4c}
\item Participant D, lines~\lineref{lne:syntax4d},~\lineref{lne:syntax4d2}
\end{itemize}
...
\begin{itemize}
\item Participant B, lines~\lineref{lne:syntax5b},~\lineref{lne:syntax5b2}
\item Participant C, lines~\lineref{lne:syntax5c},~\lineref{lne:syntax5c2},~\lineref{lne:syntax5c3},~\lineref{lne:syntax5c4}
\item Participant D, lines~\lineref{lne:syntax5d},~\lineref{lne:syntax5d2},~\lineref{lne:syntax5d3},~\lineref{lne:syntax5d4}
\item Participant E, line~\lineref{lne:syntax5e}
\item Participant F, lines~\lineref{lne:syntax5f},~\lineref{lne:syntax5f2}
\end{itemize}
\end{itemize}
...
worthwhile exercise to find ways to optimise the answer-set solver so that it
can produce its output more efficiently.

This theme appears in these lines of the transcript:

\begin{itemize}
\item Participant A, line~\lineref{lne:bug1a}
\item Participant C, line~\lineref{lne:bug1c}
\item Participant D, line~\lineref{lne:bug1d}
\item Participant E, lines~\lineref{lne:bug1e},~\lineref{lne:bug1e2}
\item Participant G, lines~\lineref{lne:bug1g},~\lineref{lne:bug1g2}
\item Participant H, lines~\lineref{lne:bug1h},~\lineref{lne:bug1h2}
\end{itemize}
...
\begin{itemize}
\item Participant A, line~\lineref{lne:bug2a}
\item Participant D, lines~\lineref{lne:bug2d},~\lineref{lne:bug2d2}
\item Participant F, line~\lineref{lne:bug2f}
\item Participant G, line~\lineref{lne:bug2g}
\end{itemize}
...
embedded as the very last event of the trope. The eventual fully-implemented
system would allow subtropes to be embedded at any point in its parent trope.

This theme appears in these lines of the transcript:
  
\begin{itemize}
\item Participant A, line~\lineref{lne:bug3a}
\item Participant B, line~\lineref{lne:bug3b}
\item Participant C, line~\lineref{lne:bug3c}
\item Participant E, line~\lineref{lne:bug3e}
\item Participant H, lines~\lineref{lne:bug3h}
\end{itemize}
...
\begin{itemize}
\item Participant B, lines~\lineref{lne:bug4b}
\item Participant G, line~\lineref{lne:bug4g}
\end{itemize}
\end{itemize}
...
line~\lineref{lne:use1a}), ``simple to use'' (Participant F,
line~\lineref{lne:use1f2}), and ``self-evident'' (Participant G, line~\lineref{lne:use1g}). Features described as making the system easy to learn
for non-programmers included the lack of a need for punctuation (Participant B, line~\lineref{lne:use1b})
...
line~\lineref{lne:use1d}). Participant E stated that the need to declare the
entities in advance may be a barrier to learning for non-programmers
(Participant E, line~\lineref{lne:use1e}). The former issue is difficult to
...
\begin{itemize}
\item Participant A, line~\lineref{lne:use1a}
\item Participant B, line~\lineref{lne:use1b}
\item Participant D, lines~\lineref{lne:use1d},~\lineref{lne:use1d2}
\item Participant E, line~\lineref{lne:use1e}
\item Participant F, lines~\lineref{lne:use1f},~\lineref{lne:use1f2}
\item Participant G, lines~\lineref{lne:use1g}
\end{itemize}
...
\begin{itemize}
\item Participant B, lines~\lineref{lne:use2b},~\lineref{lne:use2b2},~\lineref{lne:use2b3}
\item Participant E, lines~\lineref{lne:use2e},~\lineref{lne:use2e2},~\lineref{lne:use2e3}
\end{itemize}
...
that she thought about story structure (Participant C, lines~\lineref{lne:use3c}
and~\lineref{lne:use3c2}). Participant E stated that tropes are a useful way to
divide a story into modules (Particpant E, line~\lineref{lne:use3e}).
...
(Particpant F, line~\lineref{lne:use3f}).

This theme appears in these lines of the transcript:
  
\begin{itemize}
\item Participant C, lines~\lineref{lne:use3c},~\lineref{lne:use3c2}
\item Participant E, lines~\lineref{lne:use3e},~\lineref{lne:use3e2},~\lineref{lne:use3e3}
\item Participant F, line~\lineref{lne:use3f}
\end{itemize}
\end{itemize}
...
easily (Participant B, line~\lineref{lne:use1b}, Participant C,
lines~\lineref{lne:use3c} and~\lineref{lne:use3c2}). In fact, a comment from one experienced programmer participant
suggested that he had more difficulty learning the language
\emph{because} of his programing experience (Participant D,
line~\lineref{lne:use1d}). This was an interesting theme that developed among
...
lines~\lineref{lne:use3c} and~\lineref{lne:use3c2}). Other users' comments were
more focused on TropICAL's controlled natural language syntax, with Participant B
(line~\lineref{lne:use1b}) describing how its lack of ``punctuation'' (brackets
...
line~\lineref{lne:feature7g}). It is true that each trope must be defined in
...
(lines~\lineref{lne:use2b},~\lineref{lne:use2b2} and~\lineref{lne:use2b3}) describes
...
(lines~\lineref{lne:use2e},~\lineref{lne:use2e2} and~\lineref{lne:use2e3})
...
cause any confusion, because the Hero's being defined twice?\feature{Different
  characters in different tropes}\linelabel{lne:feature1a}
...
\textbf{Participant:} How does it deal with spaces?\syntax{Spaces in words}\linelabel{lne:syntax1a}
...
Lair.\feature{State management}\linelabel{lne:feature2a}

\textbf{Myself:} Yeah.

\textbf{Participant:} I'm just trying to remember the exact syntax. It's
"goes"?\syntax{Using any verb}\linelabel{lne:syntax2a}
...
with "T", that's what I've missed off.\syntax{Use of ``the''}\linelabel{lne:syntax3a}
...
is quite intuitive.\usability{Ease of use}\linelabel{lne:use1a}
...
be anywhere?\syntax{Entity definitions}\linelabel{lne:syntax4a}
...
Weapon"?\feature{Event with three entities}\linelabel{lne:feature3a}
...
wrong?\feature{State management}\linelabel{lne:feature2a2}
...
Weapon".\bug{Five event limit}\linelabel{lne:bug1a}
...
that's just a flaw in my own story!\feature{Event semantics}\linelabel{lne:feature4a}
...
just testing out very simple branching.\bug{Branch length limit}\linelabel{lne:bug2a}
...
exercise.\bug{Subtrope at end}\linelabel{lne:bug3a}
...
statement?\feature{State management}\linelabel{lne:feature2a3}
...
bit.\bug{Error messages}\linelabel{lne:bug4b}
...
\textbf{Participant:} I noticed that you've got some capitals in there, are the capitals important to the running of the program?\syntax{Capitalisation}\linelabel{lne:syntax5b}
...
Adventure" works.\syntax{Use of ``the''}\linelabel{lne:syntax3b}
...
that do I?\syntax{Using any verb}\linelabel{lne:syntax2b}
...
\textbf{Myself:} No, it should show the first event, "David drinks Coffee", but it wants the Coffee with a capital "C".\syntax{Capitalisation}\linelabel{lne:syntax5b2}
...
"role", "place" or "object".\feature{Other types of entities}\linelabel{lne:feature5b}
...
say "The Sun shines", that might be better.\feature{State management}\linelabel{lne:feature2b}
...
object, The Sun shines", see if that works.\syntax{Use of ``the''}\linelabel{lne:syntax3b2}
...
er\ldots{}\feature{Subject must be an agent}\linelabel{lne:feature6b}
...
combine tropes together.\bug{Subtrope at end}\linelabel{lne:bug3b}
...
\textbf{Participant:} I like the idea of it being able to spit out all permutations. You know, I was looking at some Google layout algorithm which compiled pictures of dinosaurs using botanical (garbled) from the past.\usability{Visualisation}\linelabel{lne:use2b}
...
\textbf{Myself:} Yeah, that's fine. I think Knife has to be capital "K".\syntax{Capitalisation}\linelabel{lne:syntax5c}
...
\textbf{Participant:} With a capital Knife? Capital K?\syntax{Capitalisation}\linelabel{lne:syntax5c2}
...
\textbf{Myself:} So it doesn't like "Knife of Immense Power", but "Knife of Power"\ldots{} it's OK if "Power" has a capital "P". So hang on, I think that's what I've done, I've said "Every word has to be capitalised". But then "of" is OK.\syntax{Capitalisation}\linelabel{lne:syntax5c3}
...
happens.\syntax{Entity declarations}\linelabel{lne:syntax4c}
...
possibilities, combining these tropes together.\syntax{Use of ``the''}\linelabel{lne:syntax3c}
...
\textbf{Participant:} Capital "Then", or\ldots{}?\syntax{Capitalisation}\linelabel{lne:syntax5c4}
...
that "arrange" tab from before.\bug{Subtrope at end}\linelabel{lne:bug3c}
...
you combine those tropes together. So you can't have any more events.\bug{Five
  event limit}\linelabel{lne:bug1c}
...
\textbf{Participant:} I'd say yes, but I think that it also helps that I am a creative writer, so I automatically think of a story pattern.\usability{Tropes}\linelabel{lne:use3c}

\textbf{Myself:} Hmm, OK.

\textbf{Participant:} So like creating a story pattern on the builder is a lot easier, because I know how to, like, combine them. So in my head, I always view that the Princess will meet the Rogue eventually. And I had to create the second trope.\usability{Tropes}\linelabel{lne:use3c2}
...
\textbf{Participant:} Yeah, but like say also in storytelling you don't reveal everything at once. So like, normally she'd be a princess until it's suddenly revealed that she's a ninja.\feature{Story structure}\linelabel{lne:feature7c}
...
\textbf{Participant:} Is it "The" Home is a place?\syntax{Use of ``the''}\linelabel{lne:syntax3d}
...
going to be OK with that?\syntax{Spaces in words}\linelabel{lne:syntax1d}
...
can you go to a second level of nesting?\bug{Branch length limit}\linelabel{lne:bug2d}
...
this. So it's kind of less predictable in a way.\usability{Ease of use}\linelabel{lne:use1d}
...
interpreter to do something clever and understand it?\syntax{Spaces in words}\linelabel{lne:syntax1d2}
...
\textbf{Myself:} And I think it has to be capital "T" as well.\syntax{Capitalisation}\linelabel{lne:syntax5d}
...
place?\feature{Subject must be an agent}\linelabel{lne:feature6d}
...
beginning.\syntax{Entity declarations}\linelabel{lne:syntax4d}
...
\textbf{Myself:} Uhh\ldots{} you can say it. It won't have any semantic meaning, but you can do two spaces, then an "Or the Villain goes nowhere". But then nowhere isn't a place. It would probably interpret that as a separate verb.\feature{Event semantics}\linelabel{lne:feature4d}
...
same name, in two different tropes? How does it handle that?\feature{Different
  characters in different tropes}\linelabel{lne:feature1d}
...
Smartphone.\syntax{Entity declarations}\linelabel{lne:syntax4d2}
...
structure. I think that makes a lot of sense, Matt.\usability{Ease of use}\linelabel{lne:use1d2}
...
declaring that they started somewhere.\feature{State management}\linelabel{lne:feature2d}
...
that'll be interesting.\feature{Subject must be an agent}\linelabel{lne:feature6d2}
...
\textbf{Myself:} There's going to have to be a capital "H". Yeah, exactly.\syntax{Capitalisation}\linelabel{lne:syntax5d4}
...
will\ldots{} that's fine.\bug{Five event limit}\linelabel{lne:bug1d}
...
like\ldots{}\feature{Event with three entities}\linelabel{lne:feature3d}

\textbf{Myself:} You can, but again it won't have any semantic\ldots{} oh yeah, it would. Kind of. It would give an agent permission to give something to another agent. So it would work, it would compile and work. Yeah, so actually that would be fine.\feature{Event semantics}\linelabel{lne:feature4d2}
...
length limit}\linelabel{lne:bug2d2}
...
things beforehand?\usability{Ease of use}\linelabel{lne:use1e}
...
this task.\bug{Five event limit}\linelabel{lne:bug1e}
...
\textbf{Myself:} Yeah, "Home" has to be capital "H". Yeah, that's right.\syntax{Capitalisation}\linelabel{lne:syntax5e}
...
wonder if it's expecting "THE Lair is a place".\syntax{Use of ``the''}\linelabel{lne:syntax3e}
...
you had.\syntax{Use of ``the''}\linelabel{lne:syntax3e2}
...
happen.\bug{Five event limit}\linelabel{lne:bug1e2}
...
end.\bug{Subtrope at end}\linelabel{lne:bug3e}
...
define, right?\feature{Other types of entities}\linelabel{lne:feature5f}
...
/ a"?\syntax{Use of ``the''}\linelabel{lne:syntax3f}
...
\textbf{Myself:} That wouldn't work, no, it has to be a capital "P".\syntax{Capitalisation}\linelabel{lne:syntax5f}
...
your character roles.\syntax{Using any verb}\linelabel{lne:syntax2f}
...
\textbf{Myself:} Oh. So if you put "The Mentor goes to the Castle", and make sure the Castle is a capital "C".\syntax{Capitalisation}\linelabel{lne:syntax5f2}
...
second step where the Hero is already at the castle.\bug{Branch length limit}\linelabel{lne:bug2f}
...
number.\feature{State management}\linelabel{lne:feature2f}
...
\textbf{Myself:} Yeah, that is a problem. And that's actually why we have this ability to put the tropes inside the tropes. So once it gets really crazy, you can just refer to another trope that already has those\ldots{}\usability{Tropes}\linelabel{lne:use3f}
...
difficult to say because I know how programming works.\usability{Ease of use}\linelabel{lne:use1f}
...
there to help them use the program.\usability{Ease of use}\linelabel{lne:use1f2}
...
so on. So you can use basically any verb in there?\syntax{Using any verb}\linelabel{lne:syntax2g}
...
to Sleeping Beauty"?\feature{Event with three entities}\linelabel{lne:feature3g}
...
\textbf{Myself:} No\ldots{} a very un-useful error.\bug{Error messages}\linelabel{lne:bug4g}
...
\textbf{Participant:} There it is. Ah\ldots{} (typing). Like this?\bug{Five
  event limit}\linelabel{lne:bug1g}
...
one action, basically?\bug{Branch length limit}\linelabel{lne:bug2g}
...
event limit includes the trope you put at the end, so click on\ldots{}\bug{Five
  event limit}\linelabel{lne:bug1g2}
...
problem.\usability{Ease of use}\linelabel{lne:use1g}
...
special happen after that?\feature{Event semantics}\linelabel{lne:feature4h}
...
\textbf{Participant:} Um, once we had talked about the fact that "die" doesn't have any special semantics, yes, that is pretty much what I expected.\feature{Event semantics}\linelabel{lne:feature4h2}
...
happens, Hero takes object".\bug{Subtrope at end}\linelabel{lne:bug3h}
...
Hello?\bug{Five event limit}\linelabel{lne:bug1h}
...
little trickier.\bug{Five event limit}\linelabel{lne:bug1h2}
...
map" there, instead of "find the map".\syntax{Use of ``the''}\linelabel{lne:syntax3h}
