...
\item This chapter, \textbf{Using TropICAL with a Multi-Agent System}, describes the integration of the stories described with
  TropICAL and StoryBuilder into a story simulation environment with multiple
  agents. The use of social institutions as story worlds is described in
  ...
  from the previous section. The chapter gives an examples of converting tropes
  to social institutions in terms of norms, and how these norms would be used to
  govern a multi-agent systems containing agents that have emotional models. This chapter is a development of \emph{Governing
...
described in terms of story tropes, appears in \Fref{sec:tropes-as-insts}.
...
caption={InstAL code resulting from the compilation of \Fref{lst:seq4}}]
find(R, T) generates
...
institution (described previously in \Fref{sec:trope-phases}). At the beginning of the trope, its state is simply \emph{active}.
...
initiation code for \Fref{lst:seq4}}]
...
termination code for \Fref{lst:seq4}}]
...
A fully compiled institution appears in \Fref{appendix:branch3} of
\Fref{appendix:instal}, which uses the following entity instances:
...
\emph{terminates} clauses. In \Fref{lst:branch-init} below, we see that three possible events are given
...
listing~\Fref{lst:obl2-gen}. \Fref{lst:obl2-init} shows the obligation
...
Now that we have determined the events that occur as part of the trope, and have
a TropICAL definition of it, we can describe its domain fluents, which include all the
...
\end{minipage}\\
The \emph{intDontTouchIt\/} institutional event gives idiot permission to break
...
in~\Fref{lst:donttouch-instal-full} of~\Fref{appendix:sausages-full}. InstAL rules like those shown here are compiled into AnsProlog, then we use the \emph{clingo} answer set solver~\citep{gebser2011potassco} to ground the program, and `solve' queries by finding all permissions and obligations that apply to any agents, given a sequence of events as the query input. The agents' percepts are then updated with their permitted and obliged actions from that time instant onwards.  Thus, the institutional model acts as a social narrative sensor, interpreting actors' actions in the context of the combination of the concrete narrative and the abstract story moves which detach (instantiate) the norms that guide the actors in the direction of the conclusion of the story arc.
...
\emph{sausages} is described in \Fref{lst:trace}. The `pj' in the trace
...
However, this section only describes one scene of the Punch and Judy show. ~\Fref{sec:pj-fullspec} describes how the full Punch and Judy story world
