...
\caption{Left: an unsolved Sudoku puzzle. Right: The solution to the puzzle. (Images from~\citep{aspsudoku})}\label{fig:sudoku-grid}
\end{figure}
As an example, we describe the creation of a Sudoku puzzle game in AnsProlog.
The code for this example is taken from~\citep{aspsudoku}.

``Sudoku'' is a famous number puzzle where numbers 1-9 are arranged on a $9
\times 9$ grid. Fig.~\Fref{fig:sudoku-grid} shows an example of a Sudoku grid. The
...
Listing~\Fref{lst:sudoku-numbers} shows AnsProlog facts that describe the layout
...
Once the layout of the Sudoku board has been described, we can begin describing
...
of the system. For example, Listing~\Fref{lst:sudoku-unique} shows the AnsProlog
...
The constraints in Listing~\Fref{lst:sudoku-constraints} describe the rules of the
...
numbers that are filled in already. Listing~\Fref{lst:sudoku-game} shows facts
...
set (shown in Listing~\Fref{lst:sudoku-solution}) consists of the game solution
in terms of numbers and the cells they are located in.
Figure~\Fref{fig:sudoku-solved} shows the Sudoku game with the solutions from
...
\begin{figure}[!t]
\centerline{\includegraphics[width=\textwidth]{sudoku-solved.png}}
...
Returning to \Fref{lst:obl1} of \Fref{sec:obl-code}, which
...
Line~\ref{line:obl1-oblev} of \Fref{lst:obl1-init} shows the
