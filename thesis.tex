...
\newcommand*{\fancyreflistlabelprefix}{list}

\fancyrefaddcaptions{english}{%
  \providecommand*{\freflistname}{listing}%
  \providecommand*{\Freflistname}{Listing}%
}

\frefformat{plain}{\fancyreflistlabelprefix}{\freflistname\fancyrefdefaultspacing#1}
\Frefformat{plain}{\fancyreflistlabelprefix}{\Freflistname\fancyrefdefaultspacing#1}

\frefformat{vario}{\fancyreflistlabelprefix}{%
  \freflistname\fancyrefdefaultspacing#1#3%
}
\Frefformat{vario}{\fancyreflistlabelprefix}{%
  \Freflistname\fancyrefdefaultspacing#1#3%
}
...
\begin{abstract}
  ...
  a programming language or narrative formalism.
  ...
  Current tools for interactive narrative
  ...
  the roles of characters through use of planners. Though these systems can
  handle player interactions and adapt the story around them, they are
  inaccessible to story authors who lack technical or programming ability.

  This thesis proposes the use of \emph{Story Tropes} to informally describe story
  components. We introduce \emph{TropICAL}, a controlled natural language system
  for the creation of tropes which
  allows non-programmer story authors to describe their story components
  informally. Inspired by Propp's \emph{Morphology}, this
  ...
  story components in terms of a formal \emph{normative framework}, and hence
  allows for the automated verification of story paths. These paths can be
  visualised as branching tree diagrams in the StoryBuilder tool, so that
  authors can visualise the effect of adding different tropes to their stories,
  aiding the process of authoring interactive narratives.

  We evaluate the suitability of these tools for interactive story construction through a
  thematic analysis of story authors’ completion of story-authoring tasks using
  ...
  }{2.1: The abstract has been rewritten to emphasise tropes as the main
    contribution, supported by tools such as TropICAL and StoryBuilder to aid
    story construction for non-programmers}
...
tropes.
More detailed explanations appear in \Fref{sec:tropes-intro} and \Fref{cha:tropes}.
...
\added{This dissertation proposes a method for authors to construct non-linear
  ...
  development environment StoryBuilder, are described in ~\Fref{cha:tropical} and~\Fref{cha:storybuilder} respectively.}{2.2.1: Added
...
\Fref{cha:evaluation}.}{2.3.3c: Rephrased to explain why we're looking at interactive narrative}
...
described in \Fref{sec:punchjudy}.

The Punch and Judy show in \Fref{sec:punchjudy} also makes use of a
...
other narrative models described in the \emph{Literature Review} chapter (\Fref{cha:literature-review}).
...
In \Fref{cha:tropical}, we describe TropICAL, our programming language
...
\changed{}{Altered outline to reflect re-ordering and addition of chapters}
...
\item A \textbf{Literature Review}.
  ...
  (narratology) from social science (\Fref{sec:narratology}), before
  examining existing implementations of interactive narrative and narrative
  generation systems from computer science
  (\Fref{sec:implementations}).
\item Using \textbf{Tropes as Story Components}:
  This chapter examines the use of \emph{tropes} as story components, analysing the process
  ...
  \Fref{cha:institutions} by showing how tropes can be used as a
  ...
  translation process.
\item We use the requirements that emerge from~\Fref{cha:literature-review}
  to~\Fref{cha:tropes}, along with those from specific use cases, to inform the design and implementation of our \textbf{TropICAL} programming
...
\item
  ...
  with the tool.
\item This chapter, \textbf{Using TropICAL with a Multi-Agent System}, describes the integration of the stories described with
  TropICAL into a working story simulation environment with multiple
  agents. The chapter gives examples of converting tropes
  to social institutions in terms of norms, and how these norms would be used to
  govern a multi-agent system containing agents that have emotional models. This chapter is a development of \emph{Governing
    Narrative Events with Institutional Norms}, which is listed as
  the \textbf{[CMN 2015]} conference paper in the list of papers below (\Fref{sec:papers}).
\item This chapter describes the \textbf{Evaluation and Validation} work done on
  the TropICAL and StoryBuilder systems. The chapter begins with a usability
  study conducted on the StoryBuilder tool, continues with a validation of the
  TropICAL system through a worked example of the full Punch and Judy story, and
  concludes with an example of ``character freedom'', where a character is able
  to break away from the constraints of the story.
\item After the work done on evaluation and validation comes the
  \textbf{Discussion} chapter. This chapter reflects on the benefits and
  limitations on our approach by comparing it with existing planner-based
  approaches from the literature review, and describes to what extent
  ``character freedom'' has been achieved. The chapter concludes with an
  evaluation against the criteria for interactive stories described in the
  ``narratology'' section of the literature review.
...
\Fref{fig:vonnegut} shows two examples of story shapes. In the ``Man in
...
overview of these implementations and their underlying research in \Fref{sec:implementations}. Has narrative theory research informed the creation of computer-generated or interactive narrative at all, or do they all take approaches grounded in computer science and artificial intelligence? If narrative theory has not been used, then we must ask: why not?
...
\changed{While interesting, L\'{e}vi-Strauss' ideas bring us no closer to
  developing a formal model of narrative structure. For that, we must go even
  further back in time, and turn to Vladimir Propp.}{Removed harsh criticism of Levi-Strauss}
...
In a typical story, one story function will follow another as the tale progresses in a sequential series of cause and effect (figure~\Fref{fig:propp1}). However, Propp's formalism also allows for simultaneous story functions to be occuring at once (figure~\Fref{fig:propp2}).
...
\changed{\Fref{cha:tropical} addresses this issue by describing how
...
grammatical generation of natural language. \Fref{fig:rumelhart} shows two
...
\Fref{fig:mimesis} shows how these components work together to form the
...
screenshot from the game is shown in \Fref{fig:facade-screen}.
...
in \Fref{sec:narrative-types}) that interactive narratives must be social
...
of an exclusion operator (``!''). \Fref{lst:exclusion1} shows an example
...
logic in \Fref{lst:exclusion1} has a type checker which ensures that no character can have multiple genders.
...
\Fref{lst:exclusion2} is another piece of \emph{Praxis} code showing how the attributes of a character called
...
\Fref{lst:ceptre-murder} shows an example from~\citet{martens2015ceptre}
...
% \Fref{sec:kripke}, where we use Interval Temporal Logic and Kripke structures to
...
\deleted{Removed reference to deleted Kripke section}
...
\Fref{cha:institutions}) because they regulate agent behaviour without
...
\Fref{sec:tropes-as-insts} contains a more detailed description of the InstAL
semantics, including an example of how a scene from the ``Punch and Judy''
story is expressed in terms of tropes, then as an InstAL institution. This example is then expressed in
InstAL syntax in \Fref{sec:arch-mas}.
...
institution described in \Fref{sec:lit-insts} is so that the characters
...
Russell's original paper proposes a model similar to that shown in \Fref{fig:circumplex}, where the $x$ axis is a person's valence level and the $y$ axis is their arousal level. He argues that the full range of human emotions lie as points along these axes. Eight such examples are shown in \Fref{fig:circumplex}.
...
dimension, and so left it in our emotional model. This is the approach we take in creating my Punch and Judy simulation, and so it is described in more detail in \Fref{sec:emotion}.
...
Though the Punch and Judy simulation described in \Fref{sec:punchjudy}
...
described in \Fref{sec:formalisms}, none have been expressive enough to
...
\Fref{sec:formalisms}, none have any means of abstraction such as this.
...
our system, described in \Fref{cha:tropes}.
...
metaphor (described in \Fref{sec:structure}, sometimes the simplest
explanations are the easiest to grasp and most enduring. We believe that our
``trope'' analogy for modelling stories (\Fref{cha:tropes}) is an
...
concept (sample size 19, see \Fref{sec:tropes-simple}). This means that tropes are a
...
by our trope-based approach to interactive narrative generation. Our approach
is an interpretation and extension of Propp's system, implemented with story
tropes and normative multi-agent system. The theory of tropes is
described in~\Fref{cha:tropes}, with ~\Fref{cha:tropical} describing the
technical implementation of the TropICAL language for story tropes. The
StoryBuilder tool, a user interface on top of TropICAL, is described in~\Fref{cha:storybuilder}}{2.2.2: Amended to give props to Propp as an influence}

% \Fref{cha:tropes} describes
% the trope concept further, with examples. \Fref{cha:institutions}
...
% \Fref{cha:tropical}. \Fref{sec:library} describes our creation of
% a library of tropes and  The thesis concludes with an evaluation
% in \Fref{sec:evaluation} and conclusions in \Fref{sec:conclusions}.
...
reuse existing story fragments. We describe this is more detail in \Fref{sec:abstraction}.

\deleted{Three paragraphs that refer to deleted goals from the literature review}

\changed{The literature review in \Fref{cha:literature-review} highlights the need
...
\Fref{cha:tropical}.}{Refers back to revised goals in the literature review}
...
\Fref{cha:introduction}. It is a template which is repeated so often in
...
\Fref{fig:evil-empire} shows a screenshot of the ``The Empire'' page on the
...
(\Fref{fig:periodic-table}) arranges the tropes into different ``groups''
...
atoms, as shown in the various examples in \Fref{fig:periodic-examples}.
...
\Fref{sec:litrev-discussion}, we now describe how tropes are suitable for use
...
The example we described in \Fref{sec:litrev-discussion}
...
\textbf{Three Act Structure} (also known as Freytag's Pyramid) shown in \Fref{fig:freytag}, which describes the shape of a
...
\Fref{sec:structure}, we can use the ``subtropes'' we just identified to
define other ``story shapes'' as described by~\citet{vonnegut2009palm}. For example, the ``man
in hole'' story shape described in \Fref{sec:structure} could simply be described as a rearrangement of the
three-act structure:
...
Table's molecule model of tropes from \Fref{fig:periodic-examples}: that
...
\emph{Trope B} occur. This is discussed further in \Fref{sec:branch-code}.
...
language (described in \Fref{cha:tropical}) for story authoring to the Oxford and London Interactive Fiction meetup group.
...
listed in \Fref{appendix:questionnaire}. The fact that all of the interactive fiction authors and games developers were
...
\Fref{cha:tropical}, where we describe the language in detail. In the same manner as a wiki, once a number of authors have contributed tropes,
...
% In \Fref{sec:pjexample}, we described the ``sausages'' scene from
...
% \end{enumerate}
% Requirements~R\Fref{perms} to~R\Fref{seqs} allow \tropical\ to describe the
% permissions, obligations and sequences of events that occur in social
% institutions, while R\Fref{conds} and~R\Fref{branches} add the ability to specify
% alternative paths through a story, as planner-based systems are able to do when
% combined with formalisms such as Propp's. Finally, requirement~R\Fref{embed}
...
% section of the next chapter (\Fref{sec:bridges}).
% \end{compactdesc}
...
\Fref{cha:literature-review}, such as those using the Mimesis
...
language syntax in \Fref{sec:t-constrained}.
\Fref{sec:t-requirements} explores the use cases for our language, deriving
a set of requirements from them. Based upon these requirements, the features and
design of the language are described in \Fref{sec:language-design}.
The translation of TropICAL to InstAL code is demonstrated in \Fref{sec:t-codegen}, with samples of generated InstAL
institutions, Answer Set
generation (\Fref{sec:t-asp}), constraints
(\Fref{sec:t-constraints}), and output traces (\Fref{sec:example-traces}). Finally, we show its extension for
the description of legal policies (\Fref{sec:t-legal}). The chapter
concludes with a summary in \Fref{sec:tropical-summary}, describing how
some requirements from \Fref{sec:requirements} were met, while others are
addressed through the \emph{StoryBuilder} tool described in \Fref{cha:storybuilder}.
...
implemented in practice, see \Fref{sec:punchjudy} of
\Fref{cha:institutions}.
...
Based on the use cases above (\Fref{sec:use-cases}), we can create the
...
\begin{itemize}
  \item Entity Declarations (\Fref{sec:dec-code})
  \item Sequences of Events (\Fref{sec:seq-code})
  \item Obligations (\Fref{sec:obl-code})
  \item Branching Events (\Fref{sec:branch-code})
  \item Subtropes (\Fref{sec:subtrope-code})
\end{itemize}
...
% definitions can be found in \Fref{appendix:tropes}.

This section begins by showing how the entities (roles, objects and places) of a
trope are defined by the author (\Fref{sec:dec-code}), then follows this with a description of how
these entities' actions are composed together to form sequences of events (\Fref{sec:seq-code}).
Normally, these events give the character entities \emph{permission} to perform
certain actions, so \Fref{sec:obl-code} shows how an author may
...
in \Fref{sec:branch-code}. Finally, the means for creating trope
abstractions by embedding existing tropes into new ones is demonstrated in
\Fref{sec:subtrope-code}.
...
behaviour. \Fref{sec:seq-code} describes how this is implemented when compiled. It is
possible to tell the agents what to do in a stronger fashion using
\emph{obligations}, which we describe in \Fref{sec:obl-code}.
...
listing~\Fref{lst:obl1}. In this case, the first action (\emph{The Hero goes
...
event has occurred. \Fref{lst:obl2} shows an example trope where the Hero
...
\ref{sec:instal-obl} on page~\pageref{sec:instal-obl} explains in detail how obligations, deadlines and
...
\Fref{sec:combining-tropes} of \Fref{cha:storybuilder}.

For technical details of the parser implementation, including the EBNF grammar
of the TropICAL language, refer to \Fref{appendix:t-grammar}.
% \Fref{appendix:parse-tree} shows visualisations of the parse tree output for some
...
InstAL, with fully translated examples of InstAL code appearing in \Fref{appendix:instal}.
...
problem to be solved, as well as its solution in terms of success criteria.
Procedural programming languages (such as Java or
Python) require the user to describe \emph{how} to solve the problem. With the
procedural approach, a \emph{Sudoku} program would consist of the steps that are taken
when a Sudoku game is played. Contrast this with the declarative programming approach,
where the program would be created by logically describing the rules of the game, which are used
...
for generating sets, and constraints that limit the sets output from the
solver to only ``answer'' sets.

The rules of an ASP program are of the following form:

\begin{lstlisting}[showstringspaces=false, label={lst:asp-rule}]
<head> :- <body> .
\end{lstlisting}

This can be read as ``head is true if body is true''. If the body part of a
clause is empty, then the clause is known as a \emph{fact}. This is the
equivalent of just the head of a rule with no conditional body following it. For example:

\begin{lstlisting}[showstringspaces=false, label={lst:asp-fact}]
hero(charlie).
\end{lstlisting}

An answer set is the minimal set of atoms that satisfy the rules 
program in an consistent manner. These atoms only appear in the answer set if a
rule or fact from the program justifies its inclusion, without relying on the answer set itself.

\subsubsection{ASP Example: A Sudoku Puzzle}
\begin{figure}[!t]
\centerline{\includegraphics[width=\textwidth]{sudoku.png}}
\caption{Left: an unsolved Sudoku puzzle. Right: The solution to the puzzle. (Images from~\citet{aspsudoku})}\label{fig:sudoku-grid}
\end{figure}
As an example, we describe the creation of a Sudoku puzzle game in AnsProlog.
The code for this example is taken from~\citet{aspsudoku}.

``Sudoku'' is a famous number puzzle where numbers 1-9 are arranged on a $9
\times 9$ grid. \Fref{fig:sudoku-grid} shows an example of a Sudoku grid. The
$9 \times 9$ board consists of 9 squares of $3 \times 3$ cells. The aim of
Sudoku is to put numbers into each cell so that each row, column and square
contains no duplicate numbers (numbers 1-9 appear only once).

In an ASP Sudoku solver, the program itself
would consist of the game rules (that numbers 1-9 are arranged in 9 $3 \times 3$ squares),
along with the constraints of the game:

\begin{itemize}
\item No number can be repeated inside a square
\item No number can be in the same column or row
\end{itemize}

\Fref{lst:sudoku-numbers} shows AnsProlog facts that describe the layout
of a Sudoku board. Line~\ref{lne:s-numbers} states that the board contains
numbers from 1 to 9, rows numbered 0 to 8 and columns also numbered 0 to 8.
Lines~\ref{lne:s-square1} to \ref{lne:s-squaren} describe the nine squares of the
Sudoku board, each containing three rows and columns of number cells, where square 0 contains rows 0 to 2, and also columns 0 to 2,
square 1 contains rows 0 to 2 and columns 3 to 5, all the way to square 8 (both rows
and columns are numbers 6 to 8).

\begin{lstlisting}[showstringspaces=false, label={lst:sudoku-numbers},
escapechar=\~, caption={AnsProlog facts for the Sudoku board}]
number(1..9). row(0..8). column(0..8).~\label{lne:s-numbers}~

square(0, 0..2, 0..2). square(1, 0..2, 3..5). square(2, 0..2, 6..8).~\label{lne:s-square1}~
square(3, 3..5, 0..2). square(4, 3..5, 3..5). square(5, 3..5, 6..8).
square(6, 6..8, 0..2). square(7, 6..8, 3..5). square(8, 6..8, 6..8).~\label{lne:s-squaren}~
\end{lstlisting}

Once the layout of the Sudoku board has been defined, we can begin describing
the rules of the game through \emph{constraints}. A constraint limits the search
space of possible solutions by describing behaviour that is not allowed as part
of the system. For example, \Fref{lst:sudoku-unique} shows the AnsProlog
rules that state that a number can only appear on a grid cell once. This works
by stating that each number from 1 to 9 can only appear at location $X, Y$ in
the grid if no other number is already there. The listing omits numbers 3 to 8
to save space.

The constraints in \Fref{lst:sudoku-constraints} describe the rules of the
game itself. Lines~\ref{lne:s-cellconst1} and~\ref{lne:s-cellconst2} state that
each number can occur only once in each row and column on the board, and
lines~\ref{lne:s-squareconst1} to~\ref{lne:s-squareconstn} describe how each
number (1 to 9) can appear only once in each of the 9 squares on the board.

\begin{lstlisting}[showstringspaces=false, label={lst:sudoku-unique},
escapechar=\~, caption={AnsProlog rules limiting one number per cell in Sudoku}]
cell(X, Y, 1) :- row(X), column(Y),~\label{lne:s-cell1}~
...
cell(X, Y, 2) :- row(X), column(Y),
                 ...
                 not cell(X, Y, 5), not cell(X, Y, 6), not cell(X, Y, 7),
                 not cell(X, Y, 8), not cell(X, Y, 9).~\label{lne:s-cell2}~

                 ...

cell(X, Y, 9) :- row(X), column(Y),~\label{lne:s-cell9}~
                 not cell(X, Y, 1), not cell(X, Y, 2), not cell(X, Y, 3),
                 not cell(X, Y, 4), not cell(X, Y, 5), not cell(X, Y, 6),
                 not cell(X, Y, 7), not cell(X, Y, 8).~\label{lne:s-celln}~
\end{lstlisting}

\begin{lstlisting}[showstringspaces=false, label={lst:sudoku-constraints},
escapechar=\~, caption={AnsProlog constraints for the rules of Sudoku}]
:- cell(X, Y1, N), cell(X, Y2, N), Y1 != Y2.~\label{lne:s-cellconst1}~
:- cell(X1, Y, N), cell(X2, Y, N), X1 != X2.~\label{lne:s-cellconst2}~

in_square(S, N) :- cell(X, Y, N), square(S, X, Y).~\label{lne:s-squareconst1}~
                :- number(N), not in_square(S, N),
                  square(S, _, _).~\label{lne:s-squareconstn}~
\end{lstlisting}

Now that we have described the rules of the game of Sudoku in AnsProlog, we can
use a \emph{solver} (Potassco's \emph{clingo}~\citep{gebser2011potassco}, in
this case) to compute models that represent its solution. Each of these models
is an \emph{answer set}, and represents a stable solution to the game. 
...
If the solver is run with just the game description, it will generate answer
sets for all possible combinations of numbers in the Sudoku board. However, a
Sudoku game begins with some of the numbers of the board already filled in. It
is up to the player to determine which numbers can fill in the rest of the grid.
In order to do this, we must add facts that describe the initial conditions of a Sudoku game: the
numbers that are filled in already. \Fref{lst:sudoku-game} shows facts
that represent a specific game of Sudoku by describing the cells that already
contain numbers. When the solver is run with this input, the resulting answer
set (shown in \Fref{lst:sudoku-solution}) consists of the game solution
in terms of numbers and the cells they are located in. \Fref{fig:sudoku-solved} shows the Sudoku game with the solutions from
the answer set as blue numbers.

\begin{lstlisting}[showstringspaces=false, label={lst:sudoku-game},
escapechar=\~, caption={AnsProlog facts for a particular game of Sudoku}]
...
\begin{lstlisting}[showstringspaces=false, label={lst:sudoku-solution},
escapechar=\~, caption={Answer set corresponding to a particular Sudoku solution}]
cell(0,0,3) cell(0,1,2) cell(0,4,8) cell(0,3,9) cell(0,5,1) cell(0,8,7)
cell(0,6,6) cell(0,2,4) cell(0,7,5)

cell(1,0,6) cell(1,1,1) cell(1,6,4) cell(1,8,9) cell(1,4,2) cell(1,7,8)
cell(1,2,7) cell(1,3,3) cell(1,5,5)

cell(2,0,8) cell(2,1,9) cell(2,4,6) cell(2,5,7) cell(2,3,4) cell(2,8,1)
cell(2,2,5) cell(2,6,3) cell(2,7,2)

cell(3,1,6) cell(3,3,1) cell(3,4,9) cell(3,6,7) cell(3,2,8) cell(3,5,2)
cell(3,0,4) cell(3,7,3) cell(3,8,5)

cell(4,2,9) cell(4,3,6) cell(4,4,5) cell(4,8,2) cell(4,0,1) cell(4,1,7)
cell(4,6,8) cell(4,5,3) cell(4,7,4)

cell(5,2,2) cell(5,7,1) cell(5,1,3) cell(5,4,7) cell(5,6,9) cell(5,8,6)
cell(5,0,5) cell(5,3,8) cell(5,5,4)

cell(6,1,5) cell(6,4,4) cell(6,8,3) cell(6,2,1) cell(6,0,9) cell(6,3,7)
cell(6,5,8) cell(6,6,2) cell(6,7,6)

cell(7,1,4) cell(7,3,2) cell(7,7,9) cell(7,8,8) cell(7,2,3) cell(7,4,1)
cell(7,0,7) cell(7,5,6) cell(7,6,5)

cell(8,1,8) cell(8,2,6) cell(8,4,3) cell(8,6,1) cell(8,0,2) cell(8,3,5)
cell(8,5,9) cell(8,7,7) cell(8,8,4)
\end{lstlisting}

\begin{figure}[!t]
\centerline{\includegraphics[width=0.5\textwidth]{sudoku-solved.png}}
\caption{The solved Sudoku board for our example. Numbers from the input are
  black, solutions are blue.}\label{fig:sudoku-solved}
\end{figure}

Prolog~\citep{clocksin2003programming} is another well-known language for
declarative programming, widely used in academia and industry.
The benefit of using ASP over Prolog comes from the fact that it is fully
declarative. This means that the ordering of rules in AnsProlog programs do not
change the meaning of the program, whereas they do in Prolog.
...
described in \Fref{sec:lit-insts}) for multi-agent systems. InstAL code
...
\changed{
Using a social institution as a story model governs character agents' actions instead of enforcing them. This allows
...
as in times of extreme emotion). This potentially gives agents more autonomy to act within the guidelines of a story,
so that their actions may generate a wider range of interesting scenarios. The
extent to which our system achieves this ``character freedom'' (if at all) is
discussed in \Fref{sec:discussion-freedom}.
}{2.1: Rephrased this to describe character freedom as interesting, but not the
  focus of the dissertation, with a
  reference to the discussion section.}

\subsubsection{Norms and Institutions}
\label{sec:norms-and-institutions}
\changed{As we describe in \Fref{sec:lit-insts} of the literature review,
  ...
  story worlds in terms of tropes, through which the actors
...
approach with our concept of \emph{story tropes}, we can describe a Propp-style formalism of narrative in terms of what agents are \emph{obliged} and \emph{permitted} to do at certain points in the story.}{Propp -> tropes}
...
\subsubsection{Describing Tropes as Institutions} % 1/2
\label{sec:tropes-as-insts}
\deleted{2.1: Argument for institutions giving characters more autonomy}
...
arc.}{2.1: Removed autonomy argument}
\changed{This section demonstrates how social institutions can be used to implement
  story tropes in a multi-agent system. Using this method, the tropes are
  described as social norms which govern the character agents of a story, where
  an institution describes the norms that govern a certain trope, and a story is
a collection of tropes.}{Explains that institutions are our
...
As \Fref{sec:instal-intro} explains, an institution describes the
...
For more details on InstAL, social institutions, and the formalism in ~\Fref{fig:events}, refer to
\Fref{sec:instal-intro} and~\citep{cliffe2007specifying}.

Using the same notation introduced in \Fref{sec:instal-intro}, \Fref{fig:events} lists some external ($\mathcal{E}_{external}$) and institutional (internal, $\mathcal{E}_{internal}$) events for the \emph{Hero's Journey} trope. A wide range of external events such as \emph{go}, \emph{meet}, \emph{kill}, \emph{escape} generate the \emph{intHerosJourney} internal event, but only if the external event meets certain criteria. These criteria could be whether or not an agent fulfils a certain role, for example. \Fref{fig:gen-hero} shows examples of such internal event generation ($\mathcal{G}$). In the first example (rule~\ref{eq:tatooine}), the \emph{intHerosJourney} event is generated when Luke Skywalker goes to Tatooine, but only if Luke has the role of \emph{hero}, and Tatooine's location is \emph{home}.
\Fref{fig:init-hero} shows how internal events initiate
...
only shows the first two events of the trope (the \emph{phase} keywords are explained further in \Fref{sec:event-phases}). Rule~\ref{eq:phaseb} shows how the \emph{intHerosJourney} internal event initiates the hero's permission to kill the villain, an obligation for the hero to go to the Land of Adventure before the villain kills the victim, and the next phase (phase C) of the \emph{Hero's Journey} trope. These fluents are only initiated if the \emph{intHerosJourney} internal event happens while the trope is in phase B (when \emph{phase(herosJourney, phaseB)} holds), however.
Fluent termination ($\mathcal{C^{\downarrow}}$) works in a similar manner to initiation, with permissions and obligations for previous trope phases being terminated once the next phase of a trope has been entered. Examples for the first two phases of the \emph{Hero's Journey} trope are shown in \Fref{fig:term-hero}.

While InstAL allows us to express tropes as social institutions, it would be
difficult to use for non-programmers who are unfamiliar with logic programming
paradigms. In order for story authors to be able to create their own tropes, a
much more user-friendly language is needed. This is the motivation for TropICAL,
and the reason we are describing tropes in that language rather than having
users write InstAL code directly.
...
Cliffe's model specifies three types of \textbf{event}: \emph{external events} (or `observed events', $\mathcal{E}_{obs}$), \emph{institutional events} ($\mathcal{E}_{instevent}$) and \emph{violation events} ($\mathcal{E}_{viol}$).
...
changes, or the triggering of story functions such as Propp's \emph{absentation} or \emph{interdiction} (described in \Fref{sec:formalisms}). 
...
A more elaborate example of an institution for the ``Punch and Judy'' story world,
described in terms of story tropes, appears in \Fref{cha:institutions}.
...
descriptions. The StoryBuilder tool described in \Fref{cha:storybuilder}
...
simulation (\Fref{sec:freedom-example} gives an example of how this kind
...
\item \emph{Obligation Fluent Declarations}: obligation fluents have three
  ...
  (line~\ref{lne:obl-fluent}, obligation events are described further in \Fref{sec:instal-obl})
...
\item \emph{Norm / fluent termination events}: this specifies which norms and
  ...
  -~\ref{lne:term-end}, described in \ref{sec:termination})
\item \emph{Generation events}: institutional events that are generated by
  ...
  described in \Fref{sec:anat-gen})
\item \emph{Norms / fluents that hold initially in the institution}: this
  ...
  (lines~\ref{lne:initially-start} -~\ref{lne:initially-end}, described in \Fref{sec:anat-initial})
\end{itemize}
...
\Fref{lst:macguffin-listing} shows a simplified ``MacGuffin'' trope in
...
\Fref{lst:macguffin-hashmap}. In the listing, a hash map is contained
...
caption={Hash Map representation of the ``MacGuffin trope from \Fref{lst:macguffin-listing}''}]
{:label "MacGuffin",
...
line~\ref{lne:instev-start} of \Fref{lst:instal-anatomy} is declared with four parameters of types
...
appears on lines~\ref{lne:init-vars} and~\ref{lne:init-end} of \Fref{lst:instal-anatomy}.
...
the components of an InstAL institution as show above and in \Fref{lst:instal-anatomy}.
...
initially:%\mylabel{t1}%
  perm(go(X, Y)) if role(X, hero),
                 place(Y, landOfAdventure),%\mylabel{t2}%
  role(lukeSkywalker, hero),%\mylabel{t3}%
  object(lightSaber, sword),%\mylabel{t4}%
  place(deathStar, landOfAdventure);%\mylabel{t5}%
\end{lstlisting}
\end{minipage}\hspace{0.08\textwidth}%\hfill
\begin{minipage}{0.3\textwidth}\scriptsize
  \myref{t1}{s1}{The following rules apply as soon as the institution is active:}
  \myref{t2}{s2}{The Hero may go to the Land of Adventure}
  \myref{t3}{s3}{``Luke Skywalker'' is a hero}
  \myref{t4}{s4}{``Lightsaber'' is a sword}
  \myref{t5}{s5}{``Death Star'' is a Land of Adventure}
\end{minipage}\\
...
As described previously in \Fref{sec:inst-events}, an institution
...
external events to occur. Returning to \Fref{lst:seq4} above, the
...
\Fref{sec:inst-events}):

\begin{minipage}{0.6\textwidth}
\begin{lstlisting}[showstringspaces=false, label={lst:instal-generates},
caption={InstAL code resulting from the compilation of \Fref{lst:seq4}},escapechar=\%]
find(R, T) generates%\mylabel{t1}%
    intSequence3(R, S, T, U, V) if%\mylabel{t2}%
        role(R, hero),%\mylabel{t3}%
        object(T, sword);%\mylabel{t4}%
kill(R, S) generates%\mylabel{t5}%
    intSequence3(R, S, T, U, V) if%\mylabel{t6}%
        role(S, villain),%\mylabel{t7}%
        role(R, hero);%\mylabel{t8}%
meet(R, S) generates%\mylabel{t9}%
    intSequence3(R, S, T, U, V) if%\mylabel{t10}%
        role(S, villain),%\mylabel{t11}%
        role(R, hero);%\mylabel{t12}%
go(R, V) generates%\mylabel{t13}%
    intSequence3(R, S, T, U, V) if%\mylabel{t14}%
        role(R, hero),%\mylabel{t15}%
        place(V, landOfAdventure);%\mylabel{t16}%
return(R, U) generates%\mylabel{t17}%
    intSequence3(R, S, T, U, V) if%\mylabel{t18}%
        role(R, hero),%\mylabel{t19}%
        place(U, home);%\mylabel{t20}%
\end{lstlisting}
\end{minipage}\hspace{0.08\textwidth}%\hfill
\begin{minipage}{0.3\textwidth}\scriptsize
  \myref{t1}{s1}{The \emph{find} external event generates...}
  \myref{t2}{s2}{...the \emph{intSequence3} institutional event if...}
  \myref{t4}{s4}{...it is the \emph{hero} that finds a \emph{sword}.}
  % \myref{t5}{s5}{The \emph{kill} external event generates...}
  % \myref{t6}{s6}{...the \emph{intSequence3} institutional event if...}
  % \myref{t8}{s8}{...it is the \emph{hero} that kills the \emph{villain}.}
  % \myref{t9}{s9}{The \emph{meet} external event generates...}
  % \myref{t10}{s10}{...the \emph{intSequence3} institutional event if...}
  % \myref{t12}{s12}{...it is the \emph{hero} that meets the \emph{villain}.}
  % \myref{t13}{s13}{The \emph{go} external event generates...}
  % \myref{t14}{s14}{...the \emph{intSequence3} institutional event if...}
  % \myref{t16}{s16}{...it is the \emph{hero} that goes to the \emph{land of adventure}.}
  % \myref{t17}{s17}{The \emph{return} external event generates...}
  % \myref{t18}{s18}{...the \emph{intSequence3} institutional event if...}
  % \myref{t20}{s20}{...it is the \emph{hero} that goes \emph{home}.}
\end{minipage}\\
...
\Fref{sec:termination}). This initiation and termination process then
...
In the case of our example sequence of events from \Fref{lst:seq4}, there
...
initiation code for \Fref{lst:seq4}}, escapechar=\%]
intSequence3(R, S, T, U, V) initiates%\mylabel{t1}%
    phase(sequence3, phaseA),%\mylabel{t2}%
    perm(find(R, T)) if%\mylabel{t3}%
        phase(sequence3, active),%\mylabel{t4}%
        role(R, hero),%\mylabel{t5}%
        object(T, sword);%\mylabel{t6}%
...
\end{minipage}\hspace{0.08\textwidth}%\hfill
\begin{minipage}{0.3\textwidth}\scriptsize
  \myref{t1}{s1}{The \emph{intSequence3} institutional event initiates the
    following:}
  \myref{t2}{s2}{The fact that we are in \emph{phase A} of the trope}
  \myref{t3}{s3}{It gives permission for the \emph{hero} to find the \emph{sword}}
  \myref{t4}{s4}{If the trope is active...}
  \myref{t6}{s6}{...and we have both a \emph{hero} and a \emph{sword} as part of
  the event.}
\end{minipage}\\
...
those initiated in \Fref{lst:initiates} is shown in
listing~\Fref{lst:terminates} below. Once an external event occurs, its
...
termination code for \Fref{lst:seq4}}, escapechar=\%]
intSequence3(R, S, T, U, V) terminates%\mylabel{t1}%
    phase(sequence3, active),%\mylabel{t2}%
    perm(go(R, V)) if%\mylabel{t3}%
        phase(sequence3, active),%\mylabel{t4}%
        role(R, hero),%\mylabel{t5}%
        place(V, landOfAdventure);%\mylabel{t6}%
...
\end{minipage}\hspace{0.08\textwidth}%\hfill
\begin{minipage}{0.3\textwidth}\scriptsize
  \myref{t1}{s1}{The \emph{intSequence3} institutional event terminates the following:}
  \myref{t2}{s2}{The fact that the trope has just become active}
  \myref{t3}{s3}{Permission for the \emph{hero} to go to the \emph{land of adventure}}
  \myref{t4}{s4}{If the trope is already active...}
  \myref{t6}{s6}{...and there is a \emph{hero} and \emph{land of adventure} as
    part of the event.}
\end{minipage}\\
...
branches shown in \Fref{lst:branch3}:
...
A fully compiled institution appears in \Fref{appendix:branch3}, which uses the following entity instances:
...
branching trope in \Fref{lst:branch3}}]
...
Returning to listing~\ref{lst:obl1} on page~\pageref{lst:obl1} of Section~\ref{sec:obl-code}, which
...
Line~\ref{line:obl1-oblev} of \ref{lst:obl1-init} on page~\pageref{lst:obl1-init} shows the
initiation rule for an obligation event with no deadline or consequence, and
listing~\Fref{lst:obl1-fluent} shows the fluent declaration for the obligation
...
\emph{intGo(hero, landOfAdventure)}. \Fref{lst:obl1-gen} shows how the
...
second (obligation) event of the trope in \Fref{lst:obl1}}, escapechar=|]
intObligation1(R, S, T) initiates
    phase(obligation1, phaseA),
    obl(intGo(R,T), intNoDeadline, noViolation), perm(go(R, T)),
                    perm(intGo(R,T)), pow(intGo(R,T)) if|\label{line:obl1-oblev}|
...
The full InstAL institution that this generates appears in \Fref{lst:obl1-instal} of appendix~\Fref{appendix:instal}.

\subsubsection{Obligations with Deadlines and Consequences}
The trope shown in \Fref{lst:obl2-trope} describes a situation where an
...
\Fref{lst:obl2-fluent} shows the fluent declaration of the obligation
event in \Fref{lst:obl2-trope}. The deadline event \emph{intKill(villain,
...
listing~\ref{lst:obl2-gen} on page~\pageref{lst:obl2-gen}. \Fref{lst:obl2-init} shows the obligation
...
the permission of the villain to kill the hero. This is shown in the code of \Fref{lst:obl2-trope}.
...
both a deadline and a consequence, translated from the trope in \Fref{lst:obl2-trope}}]
...
violation event of the trope in \Fref{lst:obl2-trope}}]
...
trope in \Fref{lst:obl2-trope}}, escapechar=|]
go(R, U) generates
    intGo(R,U) if
        role(R, hero),
        place(U, landOfAdventure);
...
listing~\Fref{lst:obl2-instal} of appendix~\Fref{appendix:instal}.
...
In \Fref{sec:subtrope-code}, we describe how \emph{subtropes} are
...
described in listings~\Fref{lst:subtrope1} and~\Fref{lst:subtrope2} can be
...
\Fref{lst:bridge-inst} shows the bridge that links the
...
\Fref{lst:bridge-source} shows the generation event corresponding to the
...
\Fref{lst:constraints} shows the AnsProlog rules used to constrain the
...
such as the one in \Fref{lst:constraints}, we can tell the answer set
...
answer sets, refer to the diagrams in \Fref{sec:trope-visualisation}.
...
\Fref{appendix:evil-empire}. The traces listed below are solved by
...
\Fref{lst:evil-trace} shows one of the four traces from the solver
...
\Fref{fig:evil-trace} shows a visualisation of the events that occur in
the answer set in \Fref{lst:evil-trace}.
...
\caption{Visualisation of the ``Evil Empire'' example trace in \Fref{lst:evil-trace}}\label{fig:evil-trace}
\end{figure}
...
institutional event on lines~\ref{lne:as-int1}, \ref{lne:as-int2} and~\ref{lne:as-int3} of \Fref{lst:evil-trace}.
...
listing~\Fref{lst:hero-trace} as an example, and a visualisation appears in \Fref{fig:hero-trace}.
...
\caption{Visualisation of the ``Hero's Journey'' example trace in \Fref{lst:hero-trace}}\label{fig:hero-trace}
\end{figure}
...
together is shown in \Fref{lst:hero-evil-trace} in
\Fref{appendix:full-trace}. A visualisation is shown in
\Fref{fig:hjee-trace} in the same appendix.
...
reader refer to the visualisations produced by the StoryBuilder tool in \Fref{sec:trope-visualisation}.


\section{Summary}
\label{sec:tropical-summary}
Returning to the requirements listed in \Fref{sec:requirements} (listed
...
\begin{enumerate}[R1.]
  \item \textbf{The software must be able to integrate into a
    multi-agent framework such as Jason}: The ``Punch and Judy'' example in \Fref{sec:punchjudy} describes
...
obligation with a strict consequence for non-compliance.
  ...
  \item \textbf{The components must be able to express sequences of events}: \Fref{sec:seq-code} shows how sequences of events can be described in
TropICAL.
  \item \textbf{The components must be able to express branching
    (diverging) events}: \Fref{sec:branch-code} describes the
  ...
  language. \Fref{sec:subtrope-code} describes the
...
visualisation of tropes. The next chapter (\Fref{cha:storybuilder})
...
R\ref{req:components} and R\ref{req:vis} in the requirements listed in \Fref{sec:requirements}:
...
of our answer set solver (as described in \Fref{sec:trope-visualisation}) to create a tree
...
\Fref{fig:sb-start} shows what the user sees when they first start the
...
is blank in this initial screenshot). \Fref{fig:sb-edit-ann} shows the
same screen as \Fref{fig:sb-start}, with the addition of annotations that
...
figure~\Fref{fig:sb-edit-ann2}, with annotations that further explain the features
...
\Fref{fig:sb-new-ann} shows the five steps that a user must take in order
to create a trope:
...
which we refer to as \emph{answer sets} or \emph{traces} in \Fref{sec:t-asp}, are lists of events that are possible in one run
...
are explained further in \Fref{sec:example-traces}.
...
\Fref{fig:sb-arrange} shows the view that the user sees when the
...
on the selected tropes. \Fref{fig:sb-combine-ann} shows the steps that a
...
shown in \Fref{fig:sb-combine-ann}, events that follow the \emph{Example
...
\caption{Two consecutive events (from \Fref{lst:seq2})}\label{fig:sb-seq1}
\end{figure}

\begin{figure}[!t]
\centerline{\includegraphics[width=\textwidth]{storybuilder-seq2.png}}
\caption{A sequence of events (from \Fref{lst:seq4})}\label{fig:sb-seq2}
\end{figure}

\subsection{Usage Examples}

The figures in this section show how the simple trope examples listed in~\Fref{sec:seq-code}
to~\Fref{sec:subtrope-code} appear when edited and visualised using the
\emph{StoryBuilder} tool.

\begin{figure}[!t]
\centerline{\includegraphics[width=\textwidth]{storybuilder-branch1.png}}
\caption{Two branches (from \Fref{lst:branch1})}\label{fig:sb-branch1}
\end{figure}

\begin{figure}[!t]
\centerline{\includegraphics[width=\textwidth]{storybuilder-branch2.png}}
\caption{Five branches (from \Fref{lst:branch2})}\label{fig:sb-branch2}
\end{figure}

To demonstrate the visualisation of sequences of events,
figure~\Fref{fig:sb-seq1} shows a sequence just two events long, with the code
from \Fref{lst:seq2} as input. \Fref{fig:sb-seq2} uses the longer
sequence of five events from \Fref{lst:seq4} as its input.

\emph{StoryBuilder}'s visualisation of branches are demonstrated in the figures
that follow. \Fref{fig:sb-branch1} shows the simple two-branch trope of
listing~\Fref{lst:branch1}. \Fref{fig:sb-branch2} shows the visualisation
of a trope with five different branches, as described by the trope in
listing~\Fref{lst:branch2}. Finally, the more complicated combination of
sequences and branches from \Fref{lst:branch3} is shown in \Fref{fig:sb-branch3}.

\begin{figure}[!t]
\centerline{\includegraphics[width=\textwidth]{storybuilder-branch3.png}}
\caption{A combination of branches and sequences (from \Fref{lst:branch3})}\label{fig:sb-branch3}
\end{figure}


\begin{figure}[!t]
\centerline{\includegraphics[width=\textwidth]{storybuilder-subtrope1.png}}
\caption{Subtrope to be embedded (from \Fref{lst:subtrope1})}\label{fig:sb-subtrope1}
\end{figure}


The final two figures in this section demonstrate the use of \emph{StoryBuilder}
for embedding subtropes inside of other tropes. \Fref{fig:sb-subtrope1} is
the \emph{StoryBuilder} visualisation that corresponds to the subtrope that is
to be embedded (described in \Fref{lst:subtrope1}). The actual embedding
of this subtrope, as described in \Fref{lst:subtrope2}, and its resulting visualisation are shown in \Fref{fig:sb-subtrope2}.

\begin{figure}[!t]
\centerline{\includegraphics[width=\textwidth]{storybuilder-subtrope2.png}}
\caption{Trope containing a subtrope (from \Fref{lst:subtrope2})}\label{fig:sb-subtrope2}
\end{figure}
...
requirements shown in \Fref{sec:requirements}:
...
\changed{}{Moved this back here from its previous position in Ch.\Fref{cha:institutions}, as this is now the first mention of P\&J}
...
\Fref{lst:audience-participation} shows a basic description of the events
...
\Fref{fig:audience-participation} shows how this trope appears when it is
...
the user interface, described in \Fref{sec:combining-tropes} below.

\Fref{lst:babysitting-trope} shows a trope to describe the
...
suitable for younger audiences. The StoryBuilder visualisation appears in \Fref{fig:babysitting-trope}.
...
\Fref{lst:sausages-trope} shows the TropICAL definition of the
...
them below. Its StoryBuilder visualisation appears in \Fref{fig:crocodile-sausages}.
...
listing~\Fref{list:absentation-trope}.
...
``Chase Scene'' trope of \Fref{list:chase-trope}.
...
repeating ourselves. \Fref{list:babysitting-trope-subtrope} shows how the
``Babysitting'' trope would be rewritten by incorporating the ``Absentation''
and ``Chase Scene'' tropes as subtropes.
\Fref{list:sausages-trope-subtrope} does the same for the ``Sausages'' trope.
...
output of the answer set solver. \Fref{fig:combining-tropes} shows the
...
\added{Further discussion on trope combination appears in \Fref{sec:trope-combination-exp}.}{2.3.2b: Forward ref to discussion on
  combining tropes together}


\chapter{Using TropICAL with a Normative Multi-Agent System}
...
% \Fref{sec:litrev-discussion} of the literature review identifies the following issues with current approaches to interactive storytelling:
...
% \end{enumerate}
This chapter describes the integration of TropICAL into a multi-agent system
consisting of multiple intelligent agents with character models.
...
This chapter begins by describing a scene from ``Punch and Judy'' in terms of
story tropes in \Fref{sec:punchjudy-tropes}. These tropes are then translated
into institutions through their expression in terms of norms (\Fref{sec:dont-touch-inst}). \Fref{sec:punchjudy} describes
an implementation of this scene using the InstAL~\citep{cliffe2007specifying} language for institutions
...
summary in \Fref{sec:inst-summary}.

\section{Punch and Judy as Tropes}
\label{sec:punchjudy-tropes}
We begin this section by building up a scene description in terms of tropes. Then we create an institution out of the scene we
...
Judy, we will describe a single scene in terms of tropes, using one piece of the
story as an illustrative example. 

The common elements of Punch and Judy are easily described in terms of Propp's
story functions. As our example, we use the scene where Punch battles a
Crocodile in order to safeguard some sausages (which we refer to as the
``Sausages Scene'').  In this scene, Joey the clown (our narrator) asks Punch to guard the sausages. Once Joey has left the stage, a Crocodile appears and eats the sausages. Punch fights with the Crocodile, but it escapes. Joey then returns to find that his sausages are gone.
...
To help with this translation from Propp to tropes, we refer to the TV Tropes page on Vladimir Propp, which maps his story functions
...
and so is discussed further only in the ``future work'' \Fref{sec:future}.
...
the \emph{or} operator (\Fref{sec:branch-code}) allowing the creation of more flexible tropes
which cater for branching story lines.

Translating a simple version of this trope into TropICAL code produces the code
in \Fref{lst:idiot-tropical}.

\begin{lstlisting}[showstringspaces=false, label={lst:idiot-tropical},
caption={The ``Don't Touch it, You Idiot!'' trope in TropICAL}]
"Dont Touch it You Idiot" is a trope where:
  The Owner is a role
  The Idiot is a role
  The Item is an object
  Away is a place
  The Owner drops the Item
...
The following listing shows the domain fluents in the institution
for the above TropICAL code, which include all the
...
\begin{align*}
  \mathcal{P} =& \left\{\mathtt{perm(drop(owner, item)), perm(go(owner, away)),}\right.\nonumber\\
...
\emph{sausages} scene for the most part, though our use of the \emph{Don't Touch It, You Idiot} trope in place of both of
Propp's \emph{interdiction} and \emph{violation} story functions introduces a
need to have two different
\emph{phases}, as explained in \Fref{sec:event-phases}. The first phase is triggered by one character warning another
...
\begin{align}
  ...
  &\qquad\qquad\qquad\qquad\text{perm}(\mathtt{drop(owner, item)})\}\label{eq:phase-inactive}\\
...
\Fref{fig:pj-phase-inits} shows how a short version of the \emph{Dont Touch
...
\Fref{fig:pj-phase-terms}. The left side of Rule~\ref{eq:phase-inactive}
...
the \emph{owner} agent to drop the \emph{item}
($\text{perm}\mathtt{(drop(owner, item))}$). Rule~\ref{eq:phase-a}
...
The termination rules in \Fref{fig:pj-phase-terms} simply terminate any
...
institution described in \Fref{appendix:sausages-full}.
...
description of how this might work appears in \Fref{sec:freedom-example}.
...
total of 27 distinct emotional states, shown in \Fref{fig:vad}.
...
states. \Fref{sec:pj-fullspec} gives a further example of an agent emotional
...
Valence, arousal and dominance values can take values of -1 (low), 0 (medium) or 1 (high). \Fref{lst:emotions} shows the emotional belief rules for an agent with medium dominance (a dominance level of 0). Note that an agent maintains beliefs about both its current emotion label (such as sleepy or happy) and the separate valence, arousal and dominance values at the same time.  Similar sets of rules handle the belief emotion for the other dominance levels.  %We address the matter of how external stimuli affect dominance in \Fref{sec:norms as percepts}.\jnote{still needs doing}

Every time an emotional variable (valence, arousal, or dominance) changes, an agent's emotion is changed according to the rules in \Fref{lst:emotions}. While an agent's valence, arousal and dominance belief values affect the way it makes decisions internally, the results of combinations of these values (sleepy, happy, etc) are broadcast as external actions. The reason for this is that an agent's emotional state may affect the way in which the character is animated: changing the speed at which they move or turning their smile into a frown, for example. For this reason, whenever an emotional change takes place, the new emotion is published as an external action of the agent so that observing entities may perceive it. The Bath sensor framework described in \Fref{sec:arch} provides the means for this evidence of the agent's internal state change to be received by the animation system and reflected accordingly in the display.
...
\Fref{lst:response} shows the AgentSpeak rules describing how an agent's
...
listing~\ref{lst:response} on page~\pageref{lst:response}, the $\texttt{+!changeMood}$ plan on
...
(\Fref{lst:response}, lines~\ref{lne:response1-start} - \ref{lne:response1-end}
...
\changed{}{Vastly reworked this section to use tropes instead of Propp for the
  sausages scene}
% How you used instAL to model Propp
To describe our institutional model, we use InstAL~\citep{cliffe2007specifying},
a domain-specific (action) language for describing institutions that compiles to
AnsProlog, a declarative programming language for Answer Set Programming (ASP).
A more detailed introduction to InstAL appears in~\Fref{sec:instal-intro}.

InstAL describes how external events
generate institutional events, which then initiate or terminate fluents that
hold at certain points in time. These fluents can include the permissions and
obligations that describe what an agent is permitted or obliged to do at any point.

For example, if an agent with the role of \emph{owner\/} leaves the stage, it generates the \emph{intDontTouchItYouIdiot\/} event in the institution, but
only if the \emph{phaseA} phase is active (i.e., the
\emph{phase(phaseA)} fluent holds, meaning that the first event in the trope has
already occurred). The InstAL code that generates the
institutional event from the external event is shown in~\Fref{lst:idiot-generate}. %\mnote{Added \emph{activeFunction} explanation}

\begin{lstlisting}[basicstyle=\scriptsize\ttfamily,showstringspaces=false, label={lst:idiot-generate},
caption={The ``Don't Touch it, You Idiot!'' trope in TropICAL}]
go(R, U) generates
    intDontTouchItYouIdiot(R, S, T, U) if
        role(R, owner),
        place(U, away);
\end{lstlisting}
% \vspace{-1cm}
The InstAL generation event is compiled to the AnsProlog code shown in~\Fref{lst:ansprolog-idiot}

% \begin{lstlisting}[basicstyle=\scriptsize\ttfamily,showstringspaces=false, label={lst:idiot-initiate},
% intDontTouchItYouIdiot(R, S, T, U) initiates
...
\begin{minipage}{0.6\textwidth}
\begin{lstlisting}[caption={},label=lst:ansprolog-idiot,basicstyle=\scriptsize\ttfamily,escapechar=\%]
occurred(intDontTouchItYouIdiot(R,S,T,U),dontTouchIt,I) :-%\mylabel{t1}%
   occurred(go(R, U),dontTouchIt,I),%\mylabel{t2}%
   holdsat(pow(pj,intDontTouchItYouIdiot(R,S,T,U)),dontTouchIt,I),%\mylabel{t3}%
   holdsat(role(R,owner),dontTouchIt,I),%\mylabel{t4}%
   holdsat(location(U,away),dontTouchIt,I),%\mylabel{t5}%
   holdsat(phase(dontTouchIt,phaseA),dontTouchIt,I), %\mylabel{t6}%
   agent(R), place(U), inst(dontTouchIt), instant(I).%\mylabel{t7}%
\end{lstlisting}
\end{minipage}\hspace{0.08\textwidth}%\hfill
\begin{minipage}{0.3\textwidth}\scriptsize
\myref{t1}{s1}{The internal \emph{intDontTouchItYouIdiot} event occurs if the following conditions are met:}
\myref{t2}{s2}{$R$ leaves the stage}
\myref{t3}{s3}{$R$ has the power to leave the stage}
\myref{t4}{s4}{$R$ has the role of owner}
\myref{t5}{s5}{$U$ has the location of away}
\myref{t6}{s6}{The \emph{phaseA} phase is active}
\myref{t7}{s7}{$R$ is an agent, $U$ is a place, dontTouchIt is an institution, $I$ is an instant}
\end{minipage}\\
The \emph{intDontTouchIt\/} institutional event gives the idiot permission to break
the item if there is one. It also terminates the permission of the absented
agent to leave the stage, as they have already done so. This is described
in~\Fref{lst:idiot-initiate}. The AnsProlog generated by the InstAL code is
shown in~\Fref{lst:idiot-initiate-asp}.

\begin{minipage}{0.6\textwidth}
\begin{lstlisting}[caption={Initiation and termination events for the second
event of the ``Don't
Touch It, You Idiot'' trope},label=lst:idiot-initiate,basicstyle=\scriptsize\ttfamily,escapechar=\%]
intDontTouchItYouIdiot(R, S, T, U) initiates
    phase(dontTouchIt, phaseB),
    perm(break(S, T)) if
        phase(dontTouchIt, phaseA),
        role(S, idiot),
        object(T, item);%\mylabel{t1}%
intDontTouchItYouIdiot(R, S, T, U) terminates
    phase(dontTouchIt, phaseA),
    perm(go(R, U)) if
        phase(dontTouchIt, phaseA),
        role(R, owner),
        place(U, away);%\mylabel{t2}%
\end{lstlisting}
\end{minipage}\hspace{0.08\textwidth}%\hfill
\begin{minipage}{0.3\textwidth}\scriptsize
  \myref{t1}{s1}{The institutional event gives the idiot permission to break the
  item}
  \myref{t2}{s2}{The institution event terminates the permission for the owner
    to leave the stage}
\end{minipage}\\

\begin{minipage}{0.6\textwidth}
\begin{lstlisting}[caption={AnsProlog code generated by the initiation code in Listing~\ref{lst:idiot-initiate}},label=lst:idiot-initiate-asp,basicstyle=\scriptsize\ttfamily,escapechar=\%]
initiated(perm(break(S, T)),dontTouchIt,I),%\mylabel{t1}%
initiated(phase(dontTouchIt, phaseB)) :-%\mylabel{t2}%
   occurred(intDontTouchItYouIdiot(R,S,T,U),dontTouchIt,I),%\mylabel{t3}%
   holdsat(phase(dontTouchIt, phaseA),dontTouchIt,I),%\mylabel{t4}%
   agent(R), object(T, item), inst(dontTouchIt), instant(I).%\mylabel{t5}%
\end{lstlisting}
\end{minipage}\hspace{0.08\textwidth}%\hfill
\begin{minipage}{0.3\textwidth}\scriptsize
\myref{t1}{s1}{The idiot gets permission to break the item}
\myref{t2}{s2}{The trope also enters phase B, if:}
\myref{t3}{s3}{The intDontTouchItYouIdiot institutional event occurs}
\myref{t4}{s4}{We are currently in phase A}
\myref{t5}{s5}{$R$ is an agent, $T$ is an object, dontTouchIt is an institution}
...
%% \end{lstlisting}
%% \end{comment}
By combining statements such as these, we can build a complete description
of the sausages scene in terms of agent norms. A full listing is shown
in~\Fref{lst:donttouch-instal-full} of Appendix~\ref{appendix:sausages-full} on page~\pageref{appendix:sausages-full}. InstAL rules like those shown here are compiled into AnsProlog, then we use the \emph{clingo} answer set solver~\citep{gebser2011potassco} to ground the program, and `solve' queries by finding all permissions and obligations that apply to any agents, given a sequence of events as the query input. The agents' percepts are then updated with their permitted and obliged actions from that time instant onwards.  Thus, the institutional model acts as a social narrative sensor, interpreting actors' actions in the context of the combination of the concrete narrative and the abstract story moves which detach (instantiate) the norms that guide the actors in the direction of the conclusion of the story arc.
...
\emph{sausages} is described in Listing~\ref{lst:trace} on page~\pageref{lst:trace}. The `pj' in the trace
...
institution, as determined by the generates relation. Listing~\ref{lst:asp} on
page~\pageref{lst:asp} shows an extract from an answer set output for the trace queried against the ASP
description of the sausages scenario, for events~1 to~2 of the scene. Starting with an initial set of fluents that hold at instant~1,
only fluents that have been initiated and not terminated hold at the next
instant. For ease of reading, some fluents such as institutional powers are
omitted from the listing. \Fref{fig:tracevis} shows a visualisation of the answer set for the trace in \Fref{lst:trace}.

\begin{figure}[!t]
\begin{minipage}{0.65\textwidth}
%% \newsavebox{\lstasp}
%% \begin{lrbox}{\lstasp}
\begin{lstlisting}[caption={Sausages scene trace from ASP},label=lst:trace,basicstyle=\scriptsize\ttfamily,label=lst:asp,escapechar=\%]
Time Step 1:
holdsat(role(punch,idiot),dontTouchItYouIdiot)%\mylabel{t1}%
holdsat(role(joey,owner),dontTouchItYouIdiot)%\mylabel{t2}%
holdsat(place(offstage,away),dontTouchItYouIdiot)%\mylabel{t3}%
holdsat(phase(dontTouchItYouIdiot,phaseA),dontTouchItYouIdiot)%\mylabel{t4}%
holdsat(object(sausages,item),dontTouchItYouIdiot)%\mylabel{t5}%
holdsat(perm(go(joey,offstage)),dontTouchItYouIdiot)%\mylabel{t6}%
occurred(intDontTouchItYouIdiot(joey,punch,sausages,offstage),dont%\mylabel{t7}%TouchItYouIdiot)
occurred(drop(joey,sausages),dontTouchItYouIdiot)%\mylabel{t8}%

Time Step 2:
holdsat(role(punch,idiot),dontTouchItYouIdiot)%\mylabel{t9}%
holdsat(role(joey,owner),dontTouchItYouIdiot)%\mylabel{t10}%
holdsat(place(offstage,away),dontTouchItYouIdiot)%\mylabel{t11}%
holdsat(phase(dontTouchItYouIdiot,phaseB),dontTouchItYouIdiot)%\mylabel{t12}%
holdsat(object(sausages,item),dontTouchItYouIdiot)%\mylabel{t13}%
holdsat(perm(break(punch,sausages)),dontTouchItYouIdiot)%\mylabel{t14}%
occurred(intDontTouchItYouIdiot(joey,punch,sausages,offstage),dontTouc%\mylabel{t15}%hItYouIdiot)
occurred(go(joey,offstage),dontTouchItYouIdiot)%\mylabel{t16}%
\end{lstlisting}
\end{minipage}\hspace{0.03\textwidth}%\hfill
% \end{lrbox}
% \noindent\usebox{\lstasp}\hfill
\begin{minipage}{0.3\textwidth}\scriptsize
  \myref{t1}{s1}{Punch is the idiot}
  \myref{t2}{s2}{Joey is the owner}
  \myref{t3}{s3}{``offstage'' the the away location}
  \myref{t4}{s4}{The trope is in phase A at instant 1}
  \myref{t5}{s5}{``sausages'' are the object}
  \myref{t6}{s6}{Joey has permission to go offstage at instant 1}
  \myref{t7}{s7}{The institutional event is triggered}
  \myref{t8}{s8}{Joey drops the sausages at instant 1}
  \myref{t12}{s12}{The trope is in phase B at instant 2}
  \myref{t15}{s14}{Punch has permission to break the sausages at instant 2}
  \myref{t16}{s16}{Joey goes offstage at instant 2}
\end{minipage}
\end{figure}
...
%     \includegraphics[width=\linewidth]{trace.pdf}
    \includegraphics[height=0.3\textheight,angle=90]{trace2-rot.pdf}
...
Our implementation of the interactive Punch and Judy show demonstrates the use
of social institutions to model narrative. 

However, this section only describes one scene of the Punch and Judy show.
Section~\Fref{sec:pj-fullspec} on page~\pageref{sec:pj-fullspec} describes how the full Punch and Judy story world
would be implemented using TropICAL, InstAL and AgentSpeak.
...
This chapter provides a full evaluation of both the TropICAL and StoryBuilder
tools. First, a user study of the StoryBuilder user interface, with discussion
and a thematic analysis of user interview transcripts, appears in
\Fref{sec:sb-eval}. Next, a validation of the TropICAL language is
described through a complete description of a Punch and Judy story in
\Fref{sec:pj-fullspec}. A discussion on how tropes are combined together
is in \Fref{sec:trope-combination-exp}, and the chapter concludes with an
examination of the extent to which ``character freedom'' has been achieved, and
the potential benefits of this, in \Fref{sec:freedom-example}.

\section{StoryBuilder Evaluation}
\label{sec:sb-eval}
% TODO intro: 2
...
by~\citet{clarke2014thematic}, and in \Fref{sec:sb-analysis}).
...
\ref{req:direct} and~\ref{req:norm} from \Fref{sec:requirements}, only
...
\Fref{sec:requirements}, and so are used as the basis of the questions
...
software. The full transcripts, with annotations, are listed in~\Fref{appendix:transcripts}.
...
\begin{itemize}
  ...
  \item Branches cannot be extended beyond one event (as described by indenting
    ...
    \Fref{lst:branch4} of \Fref{sec:branch-code}), again due to time
...
\Fref{appendix:transcripts}. These themes fall into four categories:
...
in TropICAL is described in \Fref{sec:entity-instances}. The user was able to select named characters to fulfil
...
\Fref{sec:structure}: ``Things are good, then things are bad, then things
...
be interpreted (as described in \Fref{sec:seq-code}). Once users were told this fact, they had no problem choosing
...
As explained above (\Fref{sec:limitations}), the \emph{clingo} answer-set solver that StoryBuilder uses
...
\subsection{Discussion of StoryBuilder Evaluation}
...
Though this section has evaluated the usability of the StoryBuilder tool through
a user study, the next section provides a more technical validation of the
TropICAL language by providing a fully worked example of the Punch and Judy
story, including the TropICAL code needed for each trope, their compiled InstAL
institutions, how the tropes can be combined together, and the AgentSpeak code to control the agents that interact with the
compiled institutions.

\section{A Full Specification of Punch and Judy}
\label{sec:pj-fullspec}
\added{}{2.3.2a: Added a full specification of Punch and Judy}
In \Fref{sec:punchjudy-tropes}, we described one scene from Punch and Judy
...
to talk to the audience. \Fref{lst:pjfull-narrator} shows how this trope
...
\Fref{lst:pjfull-participation} shows how this trope could be translated
...
\Fref{sec:dont-touch-inst}, this is where Judy tells Punch to look after
the baby.
...
previous section (\Fref{sec:full-introduction}).
...
Mischief'', and its TropICAL interpretation appears in \Fref{lst:pjfull-mischief}
...
wife fight with each other, as described in the TropICAL code in \Fref{lst:pjfull-domesticabuse}.
...
The slapstick trope can be defined in TropICAL with the code shown in \Fref{lst:pjfull-slapstick}.
...
The ``Chase Fight'' trope already appears in the previous scene where Punch kills his baby and wife (\Fref{lst:pjfull-chasefight}), 
...
The translation of the ``Mini-boss'' trope into TropICAL code is shown in \Fref{lst:pjfull-miniboss}.
...
\Fref{lst:pjfull-mishandling} shows a trope named ``Mishandling of an Item'' that describes the various
ways in which an item can be mishandled. Listing \ref{lst:pjfull-mischief2} on page~\pageref{lst:pjfull-mischief2} is
...
The story begins with the ``Introduction'' scene shown in \Fref{lst:pjfull-intro1}, with the character of Joey
...
\Fref{lst:pjfull-fullstory}). This is because the trope appears in every
...
\Fref{lst:pjfull-baby1} shows the definition of the ``Punch, Judy and the
...
which is shown in \Fref{lst:pjfull-police1}. In this scene, Punch has the
...
sausages. \Fref{lst:pjfull-croc1} shows the TropICAL code for the
...
\Fref{lst:pjfull-devil1}. This scene contains exactly the same tropes as
...
\Fref{lst:pjfull-fullstory} shows the TropICAL definition of the entire
...
evaluated prototype described in \Fref{cha:evaluation} does not include
...
\Fref{sec:asl-character-roles} of the appendix, with the code for each
character agent appearing in \Fref{sec:asl-character-agents}. A worked
...
\Fref{lst:pjfull-emotion} shows the shared plan library for agent emotions,
...
situation is described in \Fref{sec:freedom-example}.
...
\Fref{lst:pjfull-idiotagent} shows the AgentSpeak plans for this role.
...
\Fref{lst:pjfull-punchagent}. As well as inheriting the common code for
...
\Fref{sec:asl-character-roles} of the appendix.
...
\added{
  ...
  \Fref{sec:subtrope-code} describes the use of subtropes in TropICAL,
  ...
  \Fref{sec:bridges}. This section examines how our system allows
  ...
  (shown in \Fref{fig:sb-combine-ann}). Alternatively, if not using StoryBuilder,
  the following code in \Fref{lst:simultaneous-tropes} can be passed to
...
\end{lstlisting}
  ...
  \Fref{fig:dot-combine1} shows an example where the first events of each
  ...
  permitted or obliged to occur, as shown in \Fref{fig:dot-combine2}. Now
  ...
  \Fref{fig:dot-combine3} shows the situation after two choices from the Hero.
...
\added{}{2.3.2c: An example of ``Character Freedom''}
% Use the sausages scene
In this section we show an example where a character has the freedom to escape the main story
...
in \Fref{sec:croc-sausages-scene}), where Punch has been given the
...
``Thief'' character roles, such as the one shown in \Fref{lst:thief-trope}.
...
\Fref{lst:pjfull-directoragent}. The director agent could even make use
...
\section{Summary}
This evaluation of StoryBuilder and TropICAL has shown that while users of the
StoryBuilder system were able to use it for story construction, some
improvements to usability, discoverability, and user feedback (such as helpful
error messages) could be made. The
validation of the TropICAL language showed that it has the flexibility to
describe a full story (in this case, Punch and Judy) using controlled natural
language trope descriptions, and an example of how the system could potentially
enable character freedom is described.

The next chapter discusses the issues and limitations that arise from this
evaluation in more depth, comparing our system with the existing planner-based
approaches that appear in the literature, discussing the extent to which
``character freedom'' has been achieved, and analysing the levels of
interactivity achieved against criteria based on narrative theory.

\chapter{Discussion}
\label{cha:discussion}
\added{}{2.3.4: New ``Discussion'' chapter}
Following the evaluation of the StoryBuilder and TropICAL systems for authoring
interactive narrative, this section discusses the relative merits and
limitations of these systems against existing approaches in agent-based story
systems, and guidelines from narratology. The chapter begins with a comparison
with the planning systems for interactive storytelling described in
\Fref{sec:planner-systems}, noting the similarities and comparative
benefits and drawbacks our approach provides. Next follows a discussion in
\Fref{sec:discussion-freedom} of the extent of ``character freedom'' our
system provides, noting the limitations of our system in this respect, and the
possible benefits of extending the system to provide more freedom for the agents
to break away from the story. The chapter concludes with an analysis based on
interactive criteria from narratology, and an examination of the complexity of
the branching stories produced by using an answer set solver on the ASP
translations of different trope examples.

\section{Comparison with Planning Approaches}
\label{sec:discussion-comparison}
\added{}{2.3.3a: comparison with existing planning approaches from literature review}
...
\Fref{sec:sb-eval}, by comparing it with two
planner-based approaches mentioned in \Fref{sec:planner-systems} of the literature review: Young's
...
demonstrated in the evaluation in \Fref{sec:sb-eval}, is to allow non-programmer story authors to create
...
allow. \Fref{sec:put-together-pj} describes how such a system could be
...
\Fref{sec:normative-intro} of the introduction. One of the benefits of
...
\Fref{sec:pj-fullspec} would achieve it.

\subsection{Prototype Implementation}
The evaluation in \Fref{cha:evaluation} examined the suitability of
...
in \Fref{lst:constraints}. These violations are important to keep track
of, as they tell us that an agent is trying to break away from the permitted
course of the story.

\subsection{Proposed Full Implementation}
\Fref{sec:pj-fullspec} describes a potential implementation of TropICAL
...
This section refers back to \Fref{sec:narrative-types} of the literature
...
as we propose in \Fref{sec:topicscript}, our system could eventually meet
this definition.
...
The listing examples from \Fref{sec:language-design} are used at first to
isolate individual TropICAL features (\Fref{lst:ex-seq}
to~\Fref{lst:ex-subtrope2}), with~\Fref{lst:ex-simtrope1} to~\Fref{lst:ex-simtrope3} showing the combination of several tropes together.
...
The trope in \Fref{lst:ex-seq} is a basic sequence, five events long, one
event after another.
...
in \Fref{lst:ex-subtrope1}, which is included at the end of both active
...
\begin{lstlisting}[showstringspaces=false, label={lst:ex-simtrope1c},
...
\begin{lstlisting}[showstringspaces=false, label={lst:ex-simtrope2c},
...
\subsection{Analysis}
Based on the experiments above, the current implementation of the system is too
slow for practical usage. The major source of inefficiency that emerges from this evaluation is the generation of \emph{null} events
...
% \begin{enumerate}
% \item \Fref{cha:institutions} introduces the idea of using social
...
% \item In \Fref{cha:tropes}, we describe the use of \emph{story tropes} as a method of creating story components in
...
% \item \Fref{cha:tropical} introduces \emph{TropICAL}, our controlled natural
...
% \item \Fref{cha:storybuilder} addresses the limitations of the
...
allowing them to break away from the story in some situations. The thematic analysis of \emph{StoryBuilder} conducted in \Fref{sec:sb-eval} reveals that
...
proved effective during the study described in \Fref{sec:sb-eval}, some limitations and possibilities for
expansion emerged during the evaluation of the system. These are addressed in
\Fref{sec:future-address}. In the first Section of this chapter, however,
...
described by Vonnegut in \Fref{sec:structure}, or the ``Hero's Journey''
...
``Freytag's Pyramid'' from \Fref{sec:abstraction}, for example).
...
\Fref{sec:story-gen}. Rather than being used for dynamic, interactive
...
\Fref{sec:punchjudy}), these lines of dialogue would have to be
...
model described in \Fref{sec:emotion}, then their character role could
...
answer set solver, in the same way in which we check and generate stories in \Fref{sec:t-asp}.
...
This appendix lists the full TropICAL code for the ``Don't Touch It, You Idiot''
trope, along with the institutional model it is derived from.


\added{}{Full InstAL code for the ``Don't touch it, you idiot'' trope}
\begin{lstlisting}[showstringspaces=false,label={lst:donttouch-instal-full},caption={Full
InstAL code for the ``Don't Touch It, You Idiot'' trope, compiled from TropICAL}]
...
intDontTouchItYouIdiot(R, S, T, U) initiates
    ...
    perm(break(S, T)) if
        ...
        object(T, item);
intDontTouchItYouIdiot(R, S, T, U) initiates
    phase(dontTouchItYouIdiot, phaseC),
    perm(return(R)) if
        phase(dontTouchItYouIdiot, phaseB),
        role(R, owner);
intDontTouchItYouIdiot(R, S, T, U) initiates
...
% INITIALLY: -----------
initially
    pow(intDontTouchItYouIdiot(R, S, T, U)) if role(R, owner), role(S, idiot), object(T, item), place(U, away);
initially
    perm(intDontTouchItYouIdiot(R, S, T, U)) if role(R, owner), role(S, idiot), object(T, item), place(U, away);
initially
    perm(drop(R, T)) if role(R, owner), object(T, item);
initially
    phase(dontTouchItYouIdiot, active),
    role(joey, owner),
    role(punch, idiot),
...
\begin{align}
\langle \mathit{drop}&\mathtt{(owner, item)} \rangle\nonumber\\
                     % &\qquad\qquad\qquad
...
\begin{align}
  ...
  &\mathtt{intDontTouchItYouIdiot(owner, idiot, item, away)} \rangle\rightarrow \nonumber\\
  &\qquad\qquad\qquad\qquad\{\text{phase}(\mathtt{dontTouchItYouIdiot, phaseA}), \nonumber\\
  &\qquad\qquad\qquad\qquad\text{perm}(\mathtt{drop(owner, item)})\}\label{eq:phase-inactive-full}\\
  &\langle \{\text{phase}(\mathtt{dontTouchItYouIdiot, phaseA})\}, \nonumber\\
  &\mathtt{intDontTouchItYouIdiot(owner, idiot, item, away)} \rangle\rightarrow \nonumber\\
  &\qquad\qquad\qquad\qquad\{\text{phase}(\mathtt{dontTouchItYouIdiot, phaseB}), \nonumber\\
...
listings~\Fref{lst:ebnf1} and~\Fref{lst:ebnf2}. \Fref{lst:ebnf1} describes
the syntax for the entity declarations at the top of each trope definition file,
as described in \Fref{sec:dec-code}. \Fref{lst:ebnf2} shows the
...
\caption{Visualisation of trace for both the ``Evil Empire'' and ``Hero's Journey'' in \Fref{appendix:full-trace}}\label{fig:hjee-trace}
\end{figure}

\chapter{AgentSpeak Code for Character Agents and Roles}
\label{cha:agentspeak-roles}
\added{}{Added an appendix for agentspeak code for character and agent roles}
This appendix lists example AgentSpeak code for the various character roles in
the Punch and Judy story, along with the code for the characters themselves.
...
\begin{lstlisting}[showstringspaces=false,
label=lst:pjfull-narratoragent,caption={AgentSpeak code for the ``Narrator'' role}]
